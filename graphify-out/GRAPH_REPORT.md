# Graph Report - TexCore  (2026-09-25)

## Corpus Check
- 810 files · ~1,241,790 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 9203 nodes · 20341 edges · 593 communities (458 shown, 135 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 2495 edges (avg confidence: 0.54)
...
- axios.test.ts
- CustomUserAdmin
- MateriaPrimaAtomicidadTestCase
- InventoryConfig
- main
- Nginx API Gateway
- .generate_from_rows
...
- 0007_add_producto_intermedio.py
...
- 0012_detallepedido_cantidad_fabricada_and_more.py
...
- 9. GESTIÓN DE SERVICIOS — ITIL 4
- 1. RESUMEN EJECUTIVO
- ._ventas_despacho_cobranza
- CustomUserAdmin
- 0009_corridaproduccion_operacionproduccion_and_more.py
- 0017_formula_derivacion_y_ensayos.py
- 0011_detalleplanproduccion_corridaproduccion_detalle_plan_and_more.py
- 0015_versionado_formula.py
- .generate_next_lote_codigo
- .relacion_bano
- .duracion_minutos

## God Nodes (most connected - your core abstractions)
1. `CustomUserFactory` - 495 edges
2. `SedeFactory` - 306 edges
3. `ProductoFactory` - 237 edges
4. `cn()` - 223 edges
5. `OrdenProduccionFactory` - 206 edges
6. `AreaFactory` - 159 edges
7. `MaquinaFactory` - 139 edges
8. `BodegaFactory` - 120 edges
9. `StockBodega` - 118 edges
10. `LoteProduccionFactory` - 115 edges
...
## Communities (593 total, 135 thin omitted)

### Community 0 - "cn"
Cohesion: 0.02
Nodes (43): CustomUserFactory, ProveedorFactory, AreaViewSetTestCase, ChemicalViewSetTestCase, ChemicalViewSetExtraTestCase, ProductoViewSetMultiTenancyTestCase, ProveedorViewSetMultiTenancyTestCase, TestCase (+35 more)

### Community 2 - "Producto"
Cohesion: 0.06
Nodes (45): DetallePlanProduccion, PlanProduccion, Decimal, Plan de Producción Agregado / Reposición contra Stock (MTS).     Define metas de, Cantidad restante para cumplir la meta con producto aceptado., Desviación porcentual de la producción respecto a la meta planificada., Porcentaje de cumplimiento de primera calidad frente a la meta., Actualiza el estado según avance. (+37 more)

### Community 3 - "StockBodega"
Cohesion: 0.04
Nodes (72): AppContent(), AuditoriaDialog(), LOG_1, mockGet, DespachoDashboard(), GuiaRemisionModal(), HistorialDespachos(), BuscadorLotes() (+64 more)

### Community 4 - "types.ts"
Cohesion: 0.04
Nodes (79): AdminSedeDashboard(), ManageClientes(), ManageClientesProps, CLIENTE_1, CLIENTE_2, SelectCtx, toastErrorMock, ClienteComprasModal() (+71 more)

### Community 5 - "button.tsx"
Cohesion: 0.11
Nodes (13): OpsEstado, ProduccionKPIService, RUP - Capa de Servicio: ProduccionKPIService ===================================, Distribución de Órdenes de Producción por estado., Calcula KPIs de producción filtrados opcionalmente por sede.      Uso:         s, ProduccionKPIServiceHistorialProductoTest, ProduccionKPIServicePorProductoTest, ProduccionKPIServiceTendenciaTest (+5 more)

### Community 6 - "CustomUserFactory"
Cohesion: 0.05
Nodes (55): claseSeveridadEficiencia(), JefePlantaDashboard(), logger, toastErrorMock, toastSuccessMock, RFC-5424, UsuarioBasico, logger (+47 more)
...
Cohesion: 0.10
Nodes (66): CALIDAD_OPCIONES, StockItem, EditarMovimientoDialog(), EditarMovimientoDialogProps, EliminarMovimientoDialog(), EliminarMovimientoDialogProps, RegistrarMermaDialog(), GuiaRemisionModalProps (+58 more)

### Community 9 - "EjecutivosDashboard.tsx"
Cohesion: 0.02
Nodes (96): Command, BaseCommand, make_group_permission(), Permisos de acceso para TexCore.  Usa make_group_permission() para eliminar la d, Factory para crear clases de permiso basadas en grupos de Django.      Otorga ac, GroupSerializer, Meta, SedeSerializer (+88 more)

### Community 10 - "SedeFactory"
Cohesion: 0.03
Nodes (49): ProductoSerializer, ClienteFactory, SedeFactory, ProductoColoranteTestCase, TestCase, GIVEN: Un producto con tipo 'colorante'         WHEN: Se instancia y guarda en b, GIVEN: Múltiples productos de distintos tipos         WHEN: Se filtra por tipo ', Pruebas unitarias para la inclusión de 'colorante' ('Colorantes') en el catálogo (+41 more)
...
Cohesion: 0.02
Nodes (177): AdminSistemasDashboard(), InventoryDashboard(), InventoryDashboardProps, StockItem, KardexView, KardexViewProps, ManageAreas(), ManageAreasProps (+169 more)

### Community 14 - "signals.py"
Cohesion: 0.08
Nodes (71): ApprovalRequests(), ManageBodegas(), CALIDAD_OPCIONES, ManageProductos(), ManageQuimicos(), Group, ManageUsers(), StockViewImpl() (+63 more)
...
Cohesion: 0.04
Nodes (34): Continuidad de cadena: entrada = salida de la transformación previa., Servicio de aplicación para registrar transformaciones de producto., Registra una transformación en la OP.          El ``producto_entrada`` NO se con, TransformacionService, AreaFactory, LineaProduccionFactory, MaquinaFactory, LineaProduccionModelTestCase (+26 more)
...
Cohesion: 0.11
Nodes (13): CorridaProduccion, _crear_movimiento_inventario(), Decimal, OperacionProduccion, Revierte atómicamente una operación de producción:         - Verifica que los lo, Registra un paso unitario de transformación en máquina dentro de una corrida., GIVEN: Un lote de entrada con 500 kg en stock         WHEN: Se registra una oper, GIVEN: Entrada de 100 kg         WHEN: Salida + Merma suman 85 kg (desbalance de (+5 more)

### Community 21 - "test_serializers_extra.py"
Cohesion: 0.02
Nodes (153): react, TransformationView(), AccordionContent(), AccordionItem(), AccordionTrigger(), AlertDialog(), AlertDialogAction(), AlertDialogCancel() (+145 more)

### Community 22 - "AdminSistemasDashboard.tsx"
Cohesion: 0.07
Nodes (7): Regla 1: guardar una fórmula (congelar su receta viva) crea siempre una versión, Regla 3-4: designa una versión existente (ensayo u oficial anterior) como la, CongeladoVersionOrdenTestCase, Endpoints de §7 y reglas 1, 3-5 y 13 a través de la API., Ensayo + marcar oficial en un solo paso, para los tests que solo necesitan, Reglas 8-9 (§6): al lanzar una OP se congela la versión oficial de su fórmula y, VersionadoFormulaApiTestCase
...
Cohesion: 0.08
Nodes (44): IsInternalService, BasePermission, Permite acceso solo si el request fue autenticado como ServicePrincipal., URLs de la API interna. Namespace: internal_api., AgingView, DeudoresGerencialView, DeudoresVendedorView, KardexView (+36 more)

### Community 25 - "card.tsx"
Cohesion: 0.12
Nodes (9): AreaSerializer, DosificacionSerializer, Serializer de entrada para el endpoint de calculo de dosificacion.      Spec 202, AreaNombreValidatorTestCase, DosificacionSerializerTestCase, TestCase, validate_nombre: ALPHANUMERIC_ACCENTS_REGEX., validate_peso / validate_litros: deben ser > 0 (spec 2026-09-24 D3). (+1 more)
...
Cohesion: 0.06
Nodes (30): _apply_pragmas(), get_session_factory(), init_db(), _make_engine(), _make_session_factory(), async_sessionmaker, AsyncSession, Módulo de infraestructura de base de datos de auditoría — scanning_service. SRP: (+22 more)

### Community 28 - "pdf.py"
Cohesion: 0.05
Nodes (36): 1. Service Layer: `gestion/services/pago_reversion.py`, 1. Service Layer Pattern, 2. Backend Views: `gestion/views.py`, 2. Transactional Script Pattern, 3. Audit Trail Pattern, 3. Frontend: `VendedorDashboard.tsx`, 4. Modal: `PagoReversionModal`, 4. Template Method Pattern (Potencial) (+28 more)

### Community 29 - "locustfile.py"
Cohesion: 0.06
Nodes (53): Configuración centralizada del printing_service. Elimina dependencias de cwd (fr, build_print_record(), get_audit_repo(), AuditRepository de printing_service: persiste eventos de auditoría en SQLite. DI, Factory function — construye PrintAuditLog. SRP: separa construcción de persiste, Dependency provider para FastAPI Depends. DIP: el router no construye el repo., generate_balance_masas_pdf(), generate_etiqueta_pdf() (+45 more)

### Community 30 - "TestPdfReporteEndpoints"
Cohesion: 0.06
Nodes (36): build_scan_record(), get_audit_repo(), AuditRepository de scanning_service: persiste eventos de auditoría en SQLite. DI, Factory function — construye ScanAuditLog desde los datos del dominio.     SRP:, Dependency provider para FastAPI Depends. DIP: el router no construye el repo., AuditRepository, BackgroundTasks, Router HTTP para validación de lotes. DIP: get_validation_service y get_audit_re (+28 more)
...
Cohesion: 0.05
Nodes (39): class-variance-authority, cmdk, embla-carousel-react, dependencies, class-variance-authority, cmdk, embla-carousel-react, input-otp (+31 more)
...
Cohesion: 0.11
Nodes (11): ProduccionContinuaTestCase, TestCase, GIVEN: Una corrida activa en planta         WHEN: Se envía POST /api/corridas-pr, GIVEN: Entrada de 200 kg y salida declarada de 150 kg (desbalance 50 kg), GIVEN: Una corrida en estado 'en_proceso'         WHEN: Se invoca POST /api/corr, GIVEN: Una corrida en proceso         WHEN: Se invoca POST /api/corridas-producc, GIVEN: Una corrida con una operación registrada que consumió 100 kg y produjo 95, Pruebas para Fase 3: Modalidad 1 - Producción Continua (Desacoplamiento OP y API (+3 more)

### Community 39 - "LineaProduccionViewSetTestCase"
Cohesion: 0.06
Nodes (32): _apply_pragmas(), Base, get_session_factory(), init_db(), _make_engine(), _make_session_factory(), async_sessionmaker, AsyncSession (+24 more)
...
Cohesion: 0.08
Nodes (23): AuditLogger: registra accesos de servicios internos. ISO 27001 A.12.4 — Registro, JWTServiceAuthentication: valida JWT RS256 de microservicios. ISO 27001 A.9.4 —, Identidad de un microservicio autenticado. Inmutable post-creación.      `sede_i, ServicePrincipal, HasScope, Permisos para la API interna. ISP: una clase por responsabilidad de permiso. COB, Verifica que el ServicePrincipal tiene el scope requerido.     Uso: permission_c, HasScopeTestCase (+15 more)
...
Cohesion: 0.03
Nodes (109): AuditLog, Artefacto RUP: Módulo de Servicio Caso de Uso: CU-DescargaQuimicaAutomatica Patr, Artefacto RUP: Módulo de Servicio Caso de Uso: CU-TrazabilidadMateriaPrima (F0-0, Estado de la OP al registrar un lote (RegistroLoteService.registrar_lote), camin, DetalleHistorialDespachoAdmin, DetalleHistorialDespachoInline, HistorialDespachoAdmin, MovimientoInventarioAdmin (+101 more)
...
Cohesion: 0.13
Nodes (7): OrdenProduccionViewSet, Jefe de Planta crea orden básica: código, peso, área, Calcula detalladamente los materiales y químicos necesarios para completar la OP, ISP: Endpoint específico para tintorero para consultar stock de químicos disponi, Historial de descargas aplicadas de un químico específico, para el         panel, GET /ordenes-produccion/historial/ — filtrable por fecha, máquina, fórmula, GET /ordenes-produccion/{id}/descargas-quimico/ — descargas químicas de
...
### Community 72 - "Datos para Dashboard de Reportes Gerenciales (Ejecutivo)"
Cohesion: 0.10
Nodes (14): DefaultContentNegotiation, async_export_report(), Tarea asíncrona para generar reportes pesados sin bloquear a Gunicorn.      Igua, AsyncExportReportTestCase, TestCase, Pruebas de gestion/tasks.py — tareas asíncronas Celery.  async_export_report: co, report_path: uno de los paths whitelisteados en reporting_proxy.py (ej.     "exp, resolve_report() (+6 more)
...
Cohesion: 0.13
Nodes (12): _extract_message(), _protected_message(), Manejo de excepciones centralizado para TexCore.  Formato de respuesta de error, Extrae un mensaje legible de la estructura de datos del error DRF., Mensaje del 409 por ProtectedError; específico cuando lo bloquean órdenes de pro, Handler de excepciones unificado.     Reemplaza los 4 patrones distintos que exi, texcore_exception_handler(), ExtractMessageTestCase (+4 more)

### Community 79 - "get_session_factory"
Cohesion: 0.08
Nodes (16): Mapea cada report_path externo (el que expone inventory/reporting_proxy.py al fr, _fecha_hasta_exclusiva(), get_kardex(), get_lotes_produccion(), get_resumen_movimientos(), get_rotacion(), get_tendencia_produccion(), get_top_clientes_gerencial() (+8 more)
...
Cohesion: 0.03
Nodes (47): DjangoModelFactory, Si la máquina del lote tiene producto_merma configurado y         peso_merma > 0, DetallePedidoFactory, LoteProduccionFactory, MaquinaConMermaFactory, Meta, OperacionProduccionFactory, Params (+39 more)
...
Cohesion: 0.10
Nodes (20): 10. Fases de implementación, 11. Riesgos, 1. Problema, 2. Regla de dominio, 3. Alcance, 4. Decisiones de diseño, 5.1 Nuevo: `ProcesoTintoreria`, 5.2 Nuevo: `MaquinaProceso` (+12 more)
...
Cohesion: 0.13
Nodes (7): LoteStockAdjustmentService, Decimal, Reversión manual de stock al rechazar un lote: salida producida,         materia, SRP (barrido de higiene Fase 5.2): extraído de LoteProduccionViewSet —     la vi, Ajusta stock de salida/entrada/químicos cuando el peso neto de un lote         c, LoteStockAdjustmentTestCase, TestCase
...
Cohesion: 0.07
Nodes (32): PrintAuditLog, Base, AuditRepository, IAuditRepository, Protocol, Contrato de auditoría para printing_service., Persiste un registro de auditoría. Nunca propaga excepciones., Implementación SQLite de IAuditRepository.     SRP: solo persiste; no construye (+24 more)
...
Cohesion: 0.19
Nodes (5): BodegaSerializer, BodegaSerializerCreateTestCase, BodegaSerializerUpdateTestCase, TestCase, Pruebas de gestion/serializers/inventory_serializers.py — BodegaSerializer.  Ser
...
Cohesion: 0.08
Nodes (27): calcularSaldoAcumulado(), normalizeBodegaKey(), TransferFormData, validateTransfer(), KardexViewImpl(), ReportesViewImpl(), TransferViewImpl(), BODEGA (+19 more)
...
Cohesion: 0.07
Nodes (28): ahoraLocal(), extraerError(), RegistrarTransformacion(), mockGet, NIVEL, TrazabilidadPorCodigoPage(), NivelTrazabilidad(), mockGet (+20 more)
...
Cohesion: 0.06
Nodes (18): ProcesoTintoreriaFactory, FaseRecetaProcesoTestCase, FormulaColorProtectTestCase, FormulaFasesProcesoApiTestCase, MaquinaProcesoModelTestCase, MaquinaProcesosApiTestCase, MaquinaVolumenBanoTestCase, MigracionFasesAProcesosTestCase (+10 more)
...
Cohesion: 0.15
Nodes (16): CrearVersionDialog(), DerivarFormulaDatos, DerivarFormulaDialog(), CambiosCampos(), DiffVista(), formatoFecha(), FormulaRef, nombreDetalle() (+8 more)
...
Cohesion: 0.11
Nodes (18): BuildBalanceMasasPayloadTestCase, BuildReporteAvancePayloadTestCase, APITestCase, Función pura (sin ORM en su firma): mapea registros ya materializados     (dicts, Función pura: reconcilia stock actual + movimientos del mes por producto.     Cu, _audit(), _build_balance_masas_payload(), _build_reporte_avance_payload() (+10 more)
...
Cohesion: 0.18
Nodes (10): DespachoEstadoService, Suma, por producto_id, el peso de DetalleHistorialDespacho asignados a         e, Suma el peso requerido por producto_id a partir de los detalles del pedido., Determina qué estado debería tener el pedido AHORA MISMO según lo         realme, ProcessDespachoAPIView, APIView, Valida si un código de lote (barras) existe y tiene stock disponible., Procesa el despacho de múltiples pedidos y lotes escaneados.     Descuenta inven (+2 more)
...
Cohesion: 0.18
Nodes (5): ParseIntParamTestCase, SimpleTestCase, Pruebas de gestion/views/_common.py — helpers compartidos entre módulos de vista, parse_int_param(), Valida un query param usado como ID entero positivo (OWASP A03).      Retorna in
...
Cohesion: 0.19
Nodes (3): _parsear_rango_fechas(), Resuelve la sede a consultar respetando el aislamiento.         Retorna (sede_id, Parsea fecha_inicio/fecha_fin de query params (formato YYYY-MM-DD).     Si se om

### Community 164 - "EventoEtiquetaModelTests"
Cohesion: 0.14
...
Cohesion: 0.11
Nodes (17): 6. Backlog detallado, 7. Resumen de distribución, 8. Matriz de trazabilidad requisito ↔ historia, 9. Referencias, EP-01 · Seguridad, RBAC y Auditoría, EP-03 · Kárdex de Inventario, TEX-06 · Autenticación con JWT en cookies HttpOnly, TEX-07 · Control de acceso basado en roles para los once roles (+9 more)
...
Cohesion: 0.15
Nodes (13): Cómo ejecutar la suite, Defectos detectados y corregidos durante el refuerzo, Estado de cobertura, Fase 6 — Limpieza de `gestion/tests_integrados.py` (2026-09-02), Frontend (React), Leyenda de técnicas ISTQB, Matriz, Matriz de Trazabilidad de Pruebas — TexCore (Backend) (+5 more)
...
Cohesion: 0.02
Nodes (29): EventoEtiquetaFactory, OrdenProduccionFactory, GIVEN: Una orden de producción con producto_salida asignado         WHEN: Se cre, TestCase, Tests de OrdenProduccion — máquina de estados. Aplica técnica ISTQB: State Trans, ISTQB STT — Transiciones legales en la máquina de estados., El modelo actual permite pendiente → finalizada (no tiene validación de estado)., Tests de creación de OrdenProduccion con validaciones de campos. (+21 more)
...
Cohesion: 0.03
Nodes (74): Meta, ProveedorSerializer, ConservarOmitidosEnPutMixin, DRF 3.16 da `default=None` a los campos anulables de un unique_together (p. ej., CrearVarianteSerializer, CrearVersionSerializer, DerivarFormulaSerializer, DetalleFormulaEscrituraSerializer (+66 more)
...
Cohesion: 0.18
Nodes (6): Lectura/escritura de una transformación máquina a máquina.      ``producto_entra, TransformacionProductoSerializer, Aislamiento por área/sede: admins y jefe de planta ven todo; el         jefe de, Registra una transformación (un paso de máquina) en la OP.          Solo Jefe de, Lista las transformaciones de la OP en orden de secuencia., Devuelve el flujo completo de la OP: pasos, mermas y siguiente área.
...
Cohesion: 0.14
Nodes (11): ProduccionKPIs, Decimal, Punto de entrada único — Fachada sobre los métodos privados., Endpoint dedicado para la tendencia de 30 días (usado independientemente)., Serie diaria de kg producidos de UN producto — gráfica de drill-down (CU-EJ-09)., Genera la serie temporal completa (incluyendo días sin producción = 0)         p, Serie diaria de kg producidos, acotada a un único producto de salida., Indexa `rows` por fecha y rellena los días sin producción con kg=0. (+3 more)
...
Cohesion: 0.03
Nodes (63): FormulaColor, calcular_dosificacion_desde_snapshot(), calcular_dosificacion_gr_l(), calcular_dosificacion_pct(), DosificacionCalculator, Decimal, Servicio de calculo de dosificacion para formulas de tintoreria.  Metodos de cal, Ejecuta el calculo de dosificacion para todos los insumos de la formula. (+55 more)

### Community 253 - "development"
Cohesion: 0.06
Nodes (18): HttpUser, AdminSedeUser, AdminSistemasUser, BodegueroUser, DespachoUser, EjecutivoUser, EmpaquetadoUser, _extraer_lista() (+10 more)
...
### Community 257 - "TransferenciasInterarea.test.tsx"
Cohesion: 0.20
...
Cohesion: 0.09
Nodes (22): 17 de Septiembre de 2026, 22 de Septiembre de 2026 (portado desde `refactorizacion`/`feature`), 24 de Septiembre de 2026 — Recetas versionadas de tintorería (spec `docs/superpowers/specs/2026-09-24-recetas-versionadas-tintoreria-design.md`), 2 de Septiembre de 2026, Autenticación JWT en printing_service (hueco de seguridad encontrado en validación contra el anteproyecto capstone), Barrido de higiene del backend — Fases 1-6 completas (sesión de recuperación tras corte por tokens), Cierre de CVEs CRITICAL/HIGH (RS-11) y cobertura Trivy para los 3 microservicios satélite (RS-12), Escaneo Trivy de las 5 imágenes de producción y corrección de llave privada embebida (+14 more)
...
Cohesion: 0.04
Nodes (101): AbstractUser, BodegaAdmin, BodegueroInline, LineaProduccionAdmin, TransformacionProductoAdmin, Elimina toda la población de datos creada para pruebas (stress_test_data y seed_, Carga ~1,000,000 registros para estresar el aplicativo TexCore.  Usa bulk_create, Seed de SIMULACIÓN INTEGRAL de TexCore.  Puebla la base con un caso coherente en (+93 more)

### Community 270 - "TestValidateLatency"
Cohesion: 0.04
Nodes (42): MateriaPrimaLote, ConsumoMateriaPrimaSerializer, MateriaPrimaLoteSerializer, Meta, Entrada del endpoint registrar_entrada — la creación real la hace el servicio., RegistrarMateriaPrimaSerializer, Tarifa vigente a la fecha; vigente_hasta NULL = contrato abierto., Costo/hora vigente a la fecha; vigente_hasta NULL = sin fecha fin. (+34 more)

### Community 271 - "navigation-menu.tsx"
Cohesion: 0.25
...
### Community 273 - "Funcionalidades Faltantes para Industria Textil"
Cohesion: 0.24
...
Cohesion: 0.04
Nodes (30): Environment, PlainTextResponse, generate_zpl_label(), get_zpl_strategy(), AuditRepository, BackgroundTasks, Genera texto ZPL a partir de un template Jinja2., ZplOutputStrategy (+22 more)

### Community 275 - "10. Decisiones de Arquitectura (ADRs)"
Cohesion: 0.03
Nodes (47): BodegaFactory, StockBodegaFactory, ConsumoMezclaServiceConsumir, ConsumoMezclaServiceRevertir, ConsumoMezclaServiceRevertirMultiplesBodegas, TestCase, STT: mezcla consumida → revertir → stock restaurado., TDD — ConsumoMezclaService. COBIT DSS06 + ISO 27001 A.12.4. (+39 more)
...
### Community 305 - "CustomUserViewSet"
Cohesion: 0.33
...
Cohesion: 0.32
Nodes (5): _campos_cambiados(), _diff_detalles(), Versionado inmutable de fórmulas de color y derivación entre fórmulas (docs/supe, Diferencias de A hacia B. Las fases se emparejan por `orden` y los insumos de, GET /formula-colors/{id}/versiones/{a}/diff/{b}/ — cambios de la versión A a la
...
### Community 333 - "deploy_prod.sh script"
Cohesion: 0.40
...
### Community 343 - "📊 Monitoreo y Mantenimiento"
Cohesion: 0.33
Nodes (3): bypass_jwt(), conftest.py del printing_service. Proporciona fixtures compartidos para tests., Reemplaza jwt.decode por un mock que acepta cualquier Bearer token en tests.

### Community 344 - "🔧 Paso 1: Instalación de Dependencias Base"
Cohesion: 0.40
Nodes (3): Command, BaseCommand, Aplica las optimizaciones DDL de SQL Server (RCSI, índices, CHECK constraints) e
...
### Community 356 - "0002_fix_token_blacklist_mssql.py"
Cohesion: 0.50
...
### Community 365 - "Scripts de Utilidad"
Cohesion: 0.67
...
### Community 369 - "Docker — Configuración específica por plataforma"
Cohesion: 0.25
...
### Community 373 - "ComponenteMezclaOPSerializer"
Cohesion: 0.38
...
### Community 395 - "run_backend_tests.sh"
Cohesion: 0.40
...
### Community 398 - "Docker Compose (produccion)"
Cohesion: 0.18
...
Cohesion: 0.29
Nodes (9): construir_descripcion(), limpiar(), main(), parsear_epicas(), parsear_historias(), Arma el campo Description con narrativa + criterios + verificacion., Quita el enfasis Markdown, dejando texto plano legible en Jira., Extrae {codigo: nombre} de las cabeceras '## EP-XX - Nombre'. (+1 more)

### Community 402 - "MateriaPrimaAtomicidadTestCase"
Cohesion: 0.40
...
### Community 417 - "@radix-ui/react-aspect-ratio"
Cohesion: 0.22
...
### Community 421 - "@radix-ui/react-dropdown-menu"
Cohesion: 0.29
...
### Community 424 - "ReetiquetarModal.test.tsx"
Cohesion: 0.22
Nodes (7): LOTE, mockPost, mockPrintLabel, mockRole, SelectCtx, toastErrorMock, toastSuccessMock

### Community 425 - "@radix-ui/react-scroll-area"
Cohesion: 0.10
Nodes (16): ClienteDetailDialogImpl(), NuevaVentaDialogImpl(), calcularDiasMora(), calcularPorcentajeCredito(), calculateItemsTotal(), ItemConTotal, normalizarInputNumerico(), parseFechaPedido() (+8 more)
...
### Community 474 - ".test_saldo_resultante_kardex"
Cohesion: 0.33
...
### Community 483 - ".test_despacho_validacion_lote_sin_stock"
Cohesion: 0.33
...
### Community 566 - "produccion_kpi_service.py"
Cohesion: 0.40
...
### Community 567 - ".clean"
Cohesion: 0.12
Nodes (13): DescargaQuimicosService, Bodega, Decimal, Template Method: paso de reversión dentro de ajustar_descarga_op.         Se usa, Template Method: secuencia fija revertir → descargar.         Se usa al modifica, Servicio de descarga automática de químicos al crear/modificar/eliminar órdenes, Verifica si el stock ha caído por debajo del mínimo.         Emite warning en lo, SRP: solo ejecuta descarga inicial. Cálculo delegado a DosificacionCalculator. (+5 more)

### Community 572 - "22 de Junio de 2026"
Cohesion: 0.40
...
### Community 573 - "10. RENDIMIENTO Y BASE DE DATOS"
Cohesion: 0.40
...
### Community 574 - "7. TESTING — ESTÁNDAR ISTQB"
Cohesion: 0.25
...
### Community 575 - "8. GOBIERNO DE TI — COBIT 2019"
Cohesion: 0.40
...
### Community 580 - "13. MÉTRICAS DE DEUDA TÉCNICA"
Cohesion: 0.33
...
### Community 585 - "6. CONSISTENCIA DEL PROYECTO"
Cohesion: 0.40
...
### Community 588 - "9. GESTIÓN DE SERVICIOS — ITIL 4"
Cohesion: 0.40
...
### Community 592 - "1. RESUMEN EJECUTIVO"
Cohesion: 0.50
Nodes (4): 1. RESUMEN EJECUTIVO, Dictamen General, Puntuación por Dimensión, Resumen de Defectos — Estado Final

## Knowledge Gaps
- **2364 isolated node(s):** `name`, `version`, `private`, `@hookform/resolvers`, `@radix-ui/react-accordion` (+2359 more)
...
- **Why does `TestFechaATexto` connect `Global Constraints` to `Auditoría y Optimización de Base de Datos SQL Server 2022 — TexCore`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `ExcelFormatter` connect `Auditoría y Optimización de Base de Datos SQL Server 2022 — TexCore` to `ProductoFactory`, `Global Constraints`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Are the 142 inferred relationships involving `CustomUserFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`CustomUserFactory` has 142 INFERRED edges - model-reasoned connections that need verification._
- **Are the 134 inferred relationships involving `SedeFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`SedeFactory` has 134 INFERRED edges - model-reasoned connections that need verification._
- **Are the 106 inferred relationships involving `ProductoFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`ProductoFactory` has 106 INFERRED edges - model-reasoned connections that need verification._
- **Are the 92 inferred relationships involving `OrdenProduccionFactory` (e.g. with `ConfiguracionEmpaqueSedeModelTestCase` and `LoteProduccionPresentacionEmpaqueTestCase`) actually correct?**
  _`OrdenProduccionFactory` has 92 INFERRED edges - model-reasoned connections that need verification._
- **What connects `ASGI config for TexCore project.  It exposes the ASGI callable as a module-level`, `RFC 5424 Syslog Formatter — TexCore Backend (Django) ===========================`, `Formatter RFC 5424 para Django/Python logging.      Produce una línea por regist` to the rest of the system?**
  _3392 weakly-connected nodes found - possible documentation gaps or missing edges._