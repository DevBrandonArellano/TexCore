# Graph Report - TexCore  (2026-09-17)

## Corpus Check
- 764 files · ~1,089,967 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8318 nodes · 18360 edges · 533 communities (404 shown, 129 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 2195 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `935b57e8`
...
- CustomUserAdmin
- 0001_initial.py
- 0003_alter_pedidoventa_estado.py
- .generate_next_lote_codigo
- 0003_stockbodega_stock_comprometido_and_more.py
...
- hover-card.tsx
- next-themes
- 0009_corridaproduccion_operacionproduccion_and_more.py
- @radix-ui/react-aspect-ratio
...
1. `CustomUserFactory` - 447 edges
2. `SedeFactory` - 261 edges
3. `cn()` - 221 edges
4. `ProductoFactory` - 211 edges
5. `OrdenProduccionFactory` - 159 edges
6. `AreaFactory` - 144 edges
7. `BodegaFactory` - 117 edges
8. `StockBodega` - 115 edges
9. `LoteProduccionFactory` - 112 edges
10. `MaquinaFactory` - 111 edges
...
- `EjecucionProduccionService` --uses--> `ReservaService`  [INFERRED]
  gestion/services/ejecucion_produccion.py → inventory/services/reserva_service.py

## Import Cycles
- 1-file cycle: `TexCore/celery.py -> TexCore/celery.py`

## Communities (533 total, 129 thin omitted)

### Community 0 - "Producto"
Cohesion: 0.02
Nodes (38): CustomUserFactory, ProveedorFactory, AreaViewSetTestCase, ChemicalViewSetTestCase, ChemicalViewSetExtraTestCase, ProductoViewSetMultiTenancyTestCase, ProveedorViewSetMultiTenancyTestCase, TestCase (+30 more)

### Community 1 - "StockBodega"
Cohesion: 0.05
Nodes (45): Command, BaseCommand, Elimina toda la población de datos creada para pruebas (stress_test_data y seed_, AuditLog, DetalleHistorialDespachoAdmin, DetalleHistorialDespachoInline, HistorialDespachoAdmin, MovimientoInventarioAdmin (+37 more)

### Community 2 - "CustomUserFactory"
Cohesion: 0.05
Nodes (94): AbstractUser, BodegaAdmin, BodegueroInline, LineaProduccionAdmin, TransformacionProductoAdmin, Carga ~1,000,000 registros para estresar el aplicativo TexCore.  Usa bulk_create, Seed de SIMULACIÓN INTEGRAL de TexCore.  Puebla la base con un caso coherente en, Command (+86 more)

### Community 3 - "types.ts"
Cohesion: 0.05
Nodes (44): AppContent(), AuditoriaDialog(), LOG_1, mockGet, DespachoDashboard(), HistorialDespachos(), JefeAreaDashboard(), KpiSection (+36 more)

### Community 4 - "LoteProduccionFactory"
Cohesion: 0.13
Nodes (10): Identidad de un microservicio autenticado. Inmutable post-creación.      `sede_i, ServicePrincipal, HasScope, Permisos para la API interna. ISP: una clase por responsabilidad de permiso. COB, Verifica que el ServicePrincipal tiene el scope requerido.     Uso: permission_c, HasScopeTestCase, IsInternalServiceTestCase, TestCase (+2 more)

### Community 5 - "OrdenProduccionFactory"
Cohesion: 0.03
Nodes (28): EventoEtiquetaFactory, OrdenProduccionFactory, TestCase, Tests de OrdenProduccion — máquina de estados. Aplica técnica ISTQB: State Trans, ISTQB STT — Transiciones legales en la máquina de estados., El modelo actual permite pendiente → finalizada (no tiene validación de estado)., Tests de creación de OrdenProduccion con validaciones de campos., BVA: peso_neto_requerido = 0 debería ser inválido para una orden de producción. (+20 more)
...
Cohesion: 0.17
Nodes (11): date, ProduccionKPIService, Calcula KPIs de producción filtrados opcionalmente por sede.      Uso:         s, ProduccionKPIServiceHistorialProductoTest, ProduccionKPIServicePorProductoTest, ProduccionKPIServiceTendenciaTest, ProduccionKPIServiceTest, SimpleTestCase (+3 more)

### Community 8 - "BodegaFactory"
Cohesion: 0.11
Nodes (77): Group, AuditoriaDialogProps, EditarMovimientoDialog(), EditarMovimientoDialogProps, EliminarMovimientoDialog(), EliminarMovimientoDialogProps, RegistrarMermaDialog(), ItemIncompleto (+69 more)

### Community 9 - "EjecutivosDashboard.tsx"
Cohesion: 0.04
Nodes (50): make_group_permission(), Permisos de acceso para TexCore.  Usa make_group_permission() para eliminar la d, Factory para crear clases de permiso basadas en grupos de Django.      Otorga ac, ConsumoMezclaService, SRP: gestiona el consumo de múltiples lotes de entrada para producir un lote de, PagoReversionService, Servicio de reversión de pagos.     Responsable único: deshacer un pago y restau, TrazabilidadService — reconstruye el flujo completo de producción de una OP.  Ar (+42 more)

### Community 10 - "JefeAreaDashboard.tsx"
Cohesion: 0.03
Nodes (33): ClienteFactory, SedeFactory, ClienteAuditoriaJustificacionTestCase, TestCase, Pruebas de ClienteViewSet — requisito de justificación de auditoría en updates., TestCase, Tests de Cliente — límite de crédito. Aplica técnicas ISTQB:   - Partición de Eq, ISTQB EP — Clases de equivalencia para limite_credito:       Válida:   limite_cr (+25 more)

### Community 11 - "card.tsx"
Cohesion: 0.09
Nodes (12): ClienteListSerializer, ClienteSerializer, _fecha_pedido_to_iso_utc(), PedidoVentaResumenSerializer, PedidoVentaSerializer, DRY (barrido de higiene Fase 5.5): get_ultima_compra estaba duplicado     byte a, Serializer ligero para listados masivos (Admin/Vendedor Dashboard), Convierte fecha_pedido a ISO UTC con Z para que el frontend muestre la hora loca (+4 more)

### Community 12 - "AreaFactory"
Cohesion: 0.25
...
Cohesion: 0.02
Nodes (51): BodegaFactory, CorridaProduccionFactory, StockBodegaFactory, EjecucionProduccionServiceTestCase, TestCase, GIVEN: Dos lotes insumo A (60 kg) y B (40 kg)         WHEN: Se mezclan para prod, Pruebas unitarias y de integración para el Motor MES (EjecucionProduccionService, GIVEN: Cadena MP Proveedor -> Lote Fibra -> Lote Hilo -> Lote Tela         WHEN: (+43 more)

### Community 14 - "ProductoFactory"
Cohesion: 0.09
Nodes (34): ApprovalRequests(), KpiCard, KpiCardProps, COLORS, AreaMovementsTable(), KpiSectionImpl(), KpiSectionProps, claseSeveridadOee() (+26 more)
...
### Community 16 - "FaseRecetaFactory"
Cohesion: 0.04
Nodes (73): AdminSedeDashboard(), ManageClientes(), ManageClientesProps, CLIENTE_1, CLIENTE_2, SelectCtx, toastErrorMock, ClienteComprasModal() (+65 more)

### Community 17 - "clear_cascade_justification"
Cohesion: 0.07
Nodes (32): get_cascade_justification(), get_current_ip(), get_current_user(), _get_object_sede_id(), Validaciones de negocio que requieren contexto de auditoría.         Se llama au, Obtiene sede_id del objeto para filtrar logs de entidades eliminadas.     Priori, audit_user_delete(), audit_user_save() (+24 more)

### Community 18 - "Command"
Cohesion: 0.05
Nodes (28): Continuidad de cadena: entrada = salida de la transformación previa., Servicio de aplicación para registrar transformaciones de producto., Registra una transformación en la OP.          El ``producto_entrada`` NO se con, TransformacionService, ProductoFactory, ProductoIntermedioTestCase, TestCase, GIVEN: Un producto con tipo 'producto_intermedio'         WHEN: Se instancia y g (+20 more)

### Community 19 - "VendedorDashboard"
Cohesion: 0.05
Nodes (15): CustomUserSerializer, Ejecutivos tienen acceso a todo el dashboard de stock: asignar todas las bodegas, ComponenteMezclaOPSerializer, OrdenProduccionEstadoSerializer, DetallePedidoSerializer, ClienteSerializerValidateTieneBeneficioTestCase, ComponenteMezclaOPSerializerTestCase, CustomUserSerializerCreateUpdateTestCase (+7 more)

### Community 20 - "AuditRepository"
Cohesion: 0.03
Nodes (41): clear_cascade_justification(), Justificación para borrados en cascada (ej. DetalleFormula al borrar FormulaColo, set_cascade_justification(), Meta, ProductoSerializer, ProveedorSerializer, DetalleFormulaEscrituraSerializer, DetalleFormulaSerializer (+33 more)

### Community 21 - "NotaVentaRequest"
Cohesion: 0.02
Nodes (122): AccordionContent(), AccordionItem(), AccordionTrigger(), Avatar(), AvatarFallback(), AvatarImage(), BreadcrumbEllipsis(), BreadcrumbItem() (+114 more)
...
### Community 23 - "ProduccionKPIService"
Cohesion: 0.03
Nodes (56): MateriaPrimaLote, MateriaPrimaLoteSerializer, Meta, Entrada del endpoint registrar_entrada — la creación real la hace el servicio., RegistrarMateriaPrimaSerializer, CostoLoteService, Tarifa vigente a la fecha; vigente_hasta NULL = contrato abierto., Costo/hora vigente a la fecha; vigente_hasta NULL = sin fecha fin. (+48 more)

### Community 24 - "SedeFactory"
Cohesion: 0.07
Nodes (45): IsInternalService, BasePermission, Permite acceso solo si el request fue autenticado como ServicePrincipal., URLs de la API interna. Namespace: internal_api., AgingView, _audit(), DeudoresGerencialView, DeudoresVendedorView (+37 more)

### Community 25 - "LoteProduccionViewSet"
Cohesion: 0.06
Nodes (27): AreaSerializer, DosificacionSerializer, Serializer de entrada para el endpoint de calculo de dosificacion., calcular_dosificacion_gr_l(), calcular_dosificacion_pct(), DosificacionCalculator, Decimal, Servicio de calculo de dosificacion para formulas de tintoreria.  Metodos de cal (+19 more)

### Community 26 - "ComponenteMezclaOPFactory"
Cohesion: 0.05
...
### Community 27 - "cn"
Cohesion: 0.05
Nodes (20): Decimal, Revierte todos los ConsumoLoteDetalle de un lote_output., ComponenteMezclaOPFactory, ConsumoLoteDetalleFactory, ConsumoMezclaServiceConsumir, ConsumoMezclaServiceRevertir, ConsumoMezclaServiceRevertirMultiplesBodegas, TestCase (+12 more)

### Community 28 - "test_serializers_extra.py"
Cohesion: 0.05
...
### Community 29 - "OperarioDashboard.tsx"
Cohesion: 0.10
Nodes (41): build_print_record(), Factory function — construye PrintAuditLog. SRP: separa construcción de persiste, generate_balance_masas_pdf(), generate_etiqueta_pdf(), generate_guia_remision_pdf(), generate_historial_despachos_pdf(), generate_nota_venta_pdf(), generate_produccion_por_producto_pdf() (+33 more)

### Community 30 - "ReportService"
Cohesion: 0.06
...
### Community 31 - "pdf.py"
Cohesion: 0.11
Nodes (19): Agrupación por dominio (validada contra el archivo actual), Archivos críticos de referencia para la implementación, Archivos existentes a editar, Archivos nuevos a crear (todos en `inventory/views/`), Catálogo actual, Checklist de verificación por fase, Contexto, Decisiones ya confirmadas con el usuario (+11 more)

### Community 32 - "useKardex.ts"
Cohesion: 0.06
Nodes (33): axios, clsx, cmdk, dependencies, axios, clsx, cmdk, input-otp (+25 more)

### Community 33 - "ValidateResponse"
Cohesion: 0.23
...
### Community 34 - "_is_trusted_proxy"
Cohesion: 0.05
...
### Community 35 - "PdfOutputStrategy"
Cohesion: 0.12
Nodes (17): devDependencies, postcss, tailwindcss, terser, @testing-library/jest-dom, @testing-library/user-event, @types/jest, @types/react-dom (+9 more)

### Community 36 - "production_lote_views.py"
Cohesion: 0.05
...
### Community 37 - "PagoPermisosP017TestCase"
Cohesion: 0.12
Nodes (10): buildProps(), mockAreas, mockBodegas, mockMaquinas, mockOrdenes, mockProductos, mockSedes, renderComponent() (+2 more)

### Community 38 - "Documentación Técnica: Sistema de Reversión de Pagos del Cliente"
Cohesion: 0.08
Nodes (20): EventoEtiqueta, RegistrarLoteProduccionSerializer, EventoEtiquetaService, LoteProduccion, Copia idéntica: mantiene la version de datos vigente, solo avanza la secuencia., Cambio de datos: anula la última etiqueta vigente y emite una nueva version., _snapshot_actual(), Orquesta el registro de un lote de producción.     - Consume producto_entrada de (+12 more)
...
### Community 44 - "parse_int_param"
Cohesion: 0.06
Nodes (21): Environment, generate_zpl_label(), get_zpl_strategy(), AuditRepository, BackgroundTasks, Router para generación de etiquetas ZPL. DIP: get_zpl_strategy y get_audit_repo, Strategy Pattern para formatos de salida del printing_service. OCP: agregar un n, Genera texto ZPL a partir de un template Jinja2. (+13 more)
...
Cohesion: 0.08
Nodes (20): LogRecord, RFC 5424 Syslog Formatter — TexCore Backend (Django) ===========================, PRI = Facility × 8 + Severity., ISO 8601 UTC con precisión de milisegundos — RFC 5424 §6.2.3., MSGID derivado del nombre del logger.         RFC 5424 §6.2.7 — max 32 chars, so, Construye el campo STRUCTURED-DATA.         Incluye siempre module y lineno; mez, Construye un SD-ELEMENT RFC 5424 §6.3:             [SD-ID PARAM-NAME="PARAM-VALU, Escaping obligatorio de valores SD-PARAM — RFC 5424 §6.3.3.         Los tres car (+12 more)

### Community 47 - "_make_pedido"
Cohesion: 0.06
Nodes (25): Command, BaseCommand, Ingreso directo de stock con Kardex (COMPRA) para químicos/insumos., Un OrdenProduccionSubproceso por cada AreaProcessStep del área (estado pendiente, Replica las transiciones del OrdenProduccionSubprocesoViewSet., Transfiere un lote terminado entre áreas (con lote) + registra TransferenciaInte, Crea el despacho (HistorialDespacho + detalles + MovimientoInventario VENTA)., El tintorero crea la FormulaColor (en_pruebas -> aprobada) que usará la OP. (+17 more)

### Community 48 - "Documentación: Descarga Automática de Químicos en Tintorería"
Cohesion: 0.07
Nodes (16): BaseAuthentication, JWTServiceAuthentication, Request, Genera un JWT RS256 firmado para autenticación entre servicios.         ISO 2700, DRF Authentication backend para JWT de servicio (RS256).     Retorna None si no, JWTServiceAuthenticationExtraTestCase, TestCase, Pruebas complementarias de internal_api/authentication.py — ramas que test_authe (+8 more)

### Community 49 - ".revertir_despacho"
Cohesion: 0.12
...
### Community 52 - "pdf_produccion_views.py"
Cohesion: 0.14
...
### Community 53 - "AuditRepository"
Cohesion: 0.03
Nodes (79): KardexViewImpl(), ManageBodegas(), ManageProductos(), ManageQuimicos(), ManageUsers(), ReportesViewImpl(), StockViewImpl(), BODEGA (+71 more)

### Community 54 - "LineaProduccionViewSetTestCase"
Cohesion: 0.05
...
### Community 55 - "PrintingServiceHistorialDespachosPdfTestCase"
Cohesion: 0.06
...
### Community 56 - "ServicePrincipal"
Cohesion: 0.07
Nodes (30): AREA_1, AREA_3, BODEGA_1, BODEGA_3, CLIENTE_1, crearSedeSinSeleccionarla(), FORMULA_1, GROUP_1 (+22 more)

### Community 57 - "Documentación: Gestión de Etiquetas (Reetiquetado, Búsqueda por Fechas e Impresión Real)"
Cohesion: 0.10
...
### Community 58 - "Diseño: Microservicios Independientes — TexCore"
Cohesion: 0.13
...
### Community 60 - "resolve_sede_scope"
Cohesion: 0.07
Nodes (3): _make_service_token(), TestCase, ReportingViewsExtraTestCase

### Community 61 - "KardexParams"
Cohesion: 0.07
Nodes (30): 10 de Julio de 2026, 13 de Julio de 2026, 1 de Julio de 2026, 20 de Julio de 2026, 21 de Julio de 2026, 22 de Julio de 2026, 22 de Julio de 2026 (2), 22 de Julio de 2026 (+22 more)

### Community 62 - "4. Casos de Prueba Completos"
Cohesion: 0.09
...
### Community 63 - "Diseño: Producción Flexible con Mezclas, Transformación y Merma Vendible"
Cohesion: 0.07
...
### Community 64 - ".calcular_costo"
Cohesion: 0.03
Nodes (64): Artefacto RUP: Módulo de Servicio Caso de Uso: CU-DescargaQuimicaAutomatica Patr, EventoEtiquetaService: orquesta la numeración y creación de eventos de etiqueta., Artefacto RUP: Módulo de Servicio Caso de Uso: CU-TrazabilidadMateriaPrima (F0-0, Tests Sprint 6 (F0-001): Trazabilidad de Materia Prima. Artefacto RUP: Suite de, Meta, MovimientoInventario, Registra cada transacción de inventario. Es la fuente de verdad para la trazabil, Representa el stock actual (saldo) de un producto específico en una bodega. (+56 more)

### Community 65 - "RFC5424Formatter"
Cohesion: 0.11
...
### Community 66 - "Fase 4: Mejoras de Arquitectura y Seguridad (Propuestas)"
Cohesion: 0.19
...
### Community 67 - "LoteValidationService"
Cohesion: 0.04
Nodes (23): OrdenProduccionSerializer, Lectura/escritura de una transformación máquina a máquina.      ``producto_entra, TransformacionProductoSerializer, ParseIntParamTestCase, SimpleTestCase, Pruebas de gestion/views/_common.py — helpers compartidos entre módulos de vista, parse_int_param(), Valida un query param usado como ID entero positivo (OWASP A03).      Retorna in (+15 more)

### Community 68 - "Documentación: Reversión de Despachos con Restauración de Stock"
Cohesion: 0.07
...
### Community 69 - "AdminSistemasDashboard.test.tsx"
Cohesion: 0.07
Nodes (18): AuditMiddleware, _extract_client_ip(), _is_trusted_proxy(), Verifica si una IP pertenece a una red de proxy de confianza., Extrae la IP real del cliente de forma segura.     Solo confía en X-Forwarded-Fo, Middleware para capturar la IP y el usuario de la petición actual,     guardándo, AuditMiddlewareCallTestCase, CascadeJustificationTestCase (+10 more)

### Community 70 - "DosificacionCalculator"
Cohesion: 0.09
...
### Community 71 - ".reconcile_client_orders"
Cohesion: 0.12
...
### Community 72 - "MateriaPrimaRegistroTestCase"
Cohesion: 0.11
Nodes (15): DefaultContentNegotiation, async_export_report(), Tarea asíncrona para generar reportes pesados sin bloquear a Gunicorn.      Igua, AsyncExportReportTestCase, TestCase, Pruebas de gestion/tasks.py — tareas asíncronas Celery.  async_export_report: co, JWTServiceAuthentication: valida JWT RS256 de microservicios. ISO 27001 A.9.4 —, report_path: uno de los paths whitelisteados en reporting_proxy.py (ej.     "exp (+7 more)

### Community 73 - "ReportingViewsExtraTestCase"
Cohesion: 0.08
Nodes (25): 1. Aplicación: `gestion`, 2. Aplicación: `inventory`, 3. Aplicación: `production` y `tintura`, 4. Gestión de Despacho y Servicios Satélite, 5. Stored Procedures de Reportes de Producción, 6. Reversión de Pagos (Mayo 2026), 7. Diagramas de Proceso, `Cliente` (+17 more)

### Community 74 - "Julio 2026"
Cohesion: 0.08
...
### Community 75 - "OrdenProduccionViewSet"
Cohesion: 0.08
...
### Community 76 - "FormulaQuimicaTestCase"
Cohesion: 0.10
Nodes (6): BuildBalanceMasasPayloadTestCase, APITestCase, Función pura: reconcilia stock actual + movimientos del mes por producto.     Cu, TestPdfProduccionViews, _build_balance_masas_payload(), Traduce stock actual + movimientos del mes al schema BalanceMasasRequest     del

### Community 77 - "AnticipoClienteP1002TestCase"
Cohesion: 0.08
...
### Community 79 - "react"
Cohesion: 0.11
Nodes (13): Mapea cada report_path externo (el que expone inventory/reporting_proxy.py al fr, _fecha_hasta_exclusiva(), get_kardex(), get_lotes_produccion(), get_resumen_movimientos(), get_rotacion(), get_tendencia_produccion(), get_top_clientes_gerencial() (+5 more)

### Community 80 - "ExecutiveKPIService"
Cohesion: 0.13
...
### Community 81 - "Datos para Dashboard de Reportes Gerenciales (Ejecutivo)"
Cohesion: 0.16
...
### Community 82 - "TransicionTresFasesTestCase"
Cohesion: 0.08
...
### Community 83 - "Producción Flexible — Implementation Plan"
Cohesion: 0.08
...
### Community 84 - "Auditoría y Optimización de Base de Datos SQL Server 2022 — TexCore"
Cohesion: 0.12
Nodes (9): DetallePlanProduccion, PlanProduccion, Decimal, Plan de Producción Agregado / Reposición contra Stock (MTS).     Define metas de, Cantidad restante para cumplir la meta con producto aceptado., Desviación porcentual de la producción respecto a la meta planificada., Porcentaje de cumplimiento de primera calidad frente a la meta., Actualiza el estado según avance. (+1 more)

### Community 85 - "Esquema de Base de Datos y Modelo de Negocio"
Cohesion: 0.04
Nodes (26): AreaFactory, MaquinaFactory, ParoMaquinaFactory, KPIAreaCalidadRendimientoTestCase, KPIAreaViewTestCase, KpiEjecutivoViewTestCase, ParsearRangoFechasTestCase, ProduccionHistorialProductoViewTestCase (+18 more)

### Community 86 - "EjecutivosDashboard.test.tsx"
Cohesion: 0.09
Nodes (18): KPI_1, LOTE_1, makeQueryClient(), MAQUINA_1, MAQUINA_2, mockDelete, mockGet, mockPatch (+10 more)

### Community 87 - "dropdown-menu.tsx"
Cohesion: 0.09
...
### Community 90 - "output_strategy.py"
Cohesion: 0.06
Nodes (23): CostoLoteProduccionSerializer, LoteStockAdjustmentService, Decimal, Reversión manual de stock al rechazar un lote: salida producida,         materia, SRP (barrido de higiene Fase 5.2): extraído de LoteProduccionViewSet —     la vi, Ajusta stock de salida/entrada/químicos cuando el peso neto de un lote         c, MermaStockService, SRP: gestiona el stock de merma vendible por máquina.     ISO 27001 A.12.4: cada (+15 more)

### Community 91 - "✅ Funcionalidades Implementadas"
Cohesion: 0.15
...
### Community 92 - "ClienteSerializer"
Cohesion: 0.04
Nodes (34): Si la máquina del lote tiene producto_merma configurado y         peso_merma > 0, DetallePedidoFactory, LoteProduccionFactory, MaquinaConMermaFactory, Meta, OperacionProduccionFactory, PedidoVentaFactory, Factories para tests de TexCore — usa factory_boy.  Convención: - Cada factory c (+26 more)

### Community 93 - "engine.py"
Cohesion: 0.11
...
### Community 94 - "TexCore — Estándares de Desarrollo"
Cohesion: 0.20
...
### Community 95 - "Plan: dividir los 4 archivos "dios" del backend Django"
Cohesion: 0.18
Nodes (16): AnonRateThrottle, Serializers para la API interna. ISP: un serializer por caso de uso., ServiceTokenRefreshRequestSerializer, ServiceTokenRequestSerializer, _generate_token_pair(), _is_internal_request(), APIView, Endpoints de autenticación servicio-a-servicio. SRP: solo emite y renueva JWT de (+8 more)

### Community 96 - "JefeAreaDashboard.test.tsx"
Cohesion: 0.11
Nodes (16): CsvFormatter, DataFrame, Response, Formateador de salida CSV., Convierte DataFrames a archivos .csv descargables., generate_report(), GenerateRequest, AuditRepository (+8 more)

### Community 97 - "main.py"
Cohesion: 0.13
Nodes (12): dataframe_to_excel_bytes(), _fecha_a_texto(), _prepare_df_for_excel(), DataFrame, Convierte cualquier fecha a texto limpio: dd-mm-yyyy (solo fecha), Todo a string limpio ASCII. Fechas como dd-mm-yyyy (solo fecha)., Genera Excel con xlsxwriter. Escribe Fecha explícitamente como string ASCII., Elimina caracteres no imprimibles que causan iconos de error en Excel. (+4 more)

### Community 98 - "Agosto 2026"
Cohesion: 0.10
...
### Community 100 - "Recursos Algorítmicos y Técnicas de Optimización en TexCore"
Cohesion: 0.10
...
### Community 103 - "Plan de Estabilización, Consolidadación y Despliegue a Producción — TexCore"
Cohesion: 0.19
...
### Community 105 - "TintoreroRBACTestCase"
Cohesion: 0.12
...
### Community 107 - "ReportingProxyRBACtest"
Cohesion: 0.11
...
### Community 108 - "RBACMatrixTestCase"
Cohesion: 0.11
...
### Community 110 - "Guía de Despliegue en Producción — TexCore"
Cohesion: 0.11
Nodes (17): ALERTA_1, BODEGA_1, BODEGA_2, BODEGA_3, LOTE_1, mockDelete, mockGet, mockHandleExport (+9 more)

### Community 111 - "3. Puntos Críticos de Comparación"
Cohesion: 0.20
...
### Community 112 - "3. Tabla resumen de escalones probados"
Cohesion: 0.17
Nodes (6): BodegaSerializer, Meta, BodegaSerializerCreateTestCase, BodegaSerializerUpdateTestCase, TestCase, Pruebas de gestion/serializers/inventory_serializers.py — BodegaSerializer.  Ser
...
### Community 115 - ".revertir_pago"
Cohesion: 0.13
Nodes (11): Configuración centralizada del printing_service. Elimina dependencias de cwd (fr, init_db(), Crea las tablas y aplica permisos de archivo.     ISO 27001 A.10: chmod 0o600 —, Escape backslash, double quote, and right bracket as per RFC 5424 §6.3.3, RFC 5424 Syslog Formatter     Format: <PRI>VERSION TIMESTAMP HOSTNAME APP-NAME P, RFC5424Formatter, lifespan(), FastAPI (+3 more)
...
### Community 122 - "Plan: dividir los 4 archivos "dios" del backend Django (v2 — revalidado contra el código actual)"
Cohesion: 0.20
...
### Community 123 - "Auditoría y división de los 6 dashboards "dios" del frontend (SOLID / Clean Code / ISO 25010)"
Cohesion: 0.08
Nodes (21): Bodega, LoteProduccion, OrdenProduccion, Producto, Domain models: objetos de dominio puros, sin acoplamiento a ORM ni HTTP. DIP: Lo, StockBodega, DjangoApiClient, LoteProduccion (+13 more)

### Community 124 - "menubar.tsx"
Cohesion: 0.11
...
### Community 125 - "VendedorDashboard.detalle.test.tsx"
Cohesion: 0.25
Nodes (5): OpsEstado, ProduccionKPIs, RUP - Capa de Servicio: ProduccionKPIService ===================================, Distribución de Órdenes de Producción por estado., Contrato de salida del servicio. Inmutable para garantizar consistencia     entr

### Community 126 - "CookieJWTAuthentication"
Cohesion: 0.29
Nodes (7): scripts, build, dev, preview, test, test:coverage, test:watch

### Community 127 - "custom_jwt_views.py"
Cohesion: 0.16
...
### Community 128 - "BodegaSerializer"
Cohesion: 0.07
...
### Community 129 - "CHANGELOG.md"
Cohesion: 0.04
Nodes (39): DjangoModelFactory, DescargaQuimicosService, Bodega, Decimal, Template Method: paso de reversión dentro de ajustar_descarga_op.         Se usa, Template Method: secuencia fija revertir → descargar.         Se usa al modifica, Servicio de descarga automática de químicos al crear/modificar/eliminar órdenes, Verifica si el stock ha caído por debajo del mínimo.         Emite warning en lo (+31 more)

### Community 130 - "Microservicios Independientes — Plan de Implementación"
Cohesion: 0.22
...
Cohesion: 0.04
Nodes (42): OeeService, Decimal, OeeService: calcula el Overall Equipment Effectiveness (OEE) por máquina o área., PrintingServiceGuiaRemisionPdfTestCase, PrintingServiceHistorialDespachosPdfTestCase, PrintingServiceLabelPdfTestCase, PrintingServiceNotaVentaPdfTestCase, PrintingServiceProduccionPorProductoPdfTestCase (+34 more)

### Community 132 - "ManageUsers.test.tsx"
Cohesion: 0.26
...
### Community 133 - "OperarioDashboard.test.tsx"
Cohesion: 0.17
...
### Community 135 - "test_signals.py"
Cohesion: 0.22
Nodes (8): eslintConfig, extends, name, private, type, version, react-app, react-app/jest

### Community 136 - "Auditoría y Mejoras del Rol Jefe de Área"
Cohesion: 0.12
...
### Community 139 - "TransformacionAPIViewTestCase"
Cohesion: 0.16
Nodes (8): LoteProduccionSerializer, LoteProduccionSerializerValidateDesviacionOrdenTestCase, LoteProduccionSerializerValidateEmpaquetadoTestCase, _payload(), TestCase, Pruebas de gestion/serializers/production_serializers.py — LoteProduccionSeriali, CustomUserViewSet, Lista vendedores para filtros en dashboards (ejecutivo/admin).

### Community 140 - "Servicio Satélite de Reportes Excel — TexCore"
Cohesion: 0.15
...
### Community 141 - "_fecha_a_texto"
Cohesion: 0.18
Nodes (5): _create_credential(), TestCase, Tests para endpoints de autenticación de servicios. EP + STT., TestServiceTokenView, TestServiceTokenViewThrottle

### Community 142 - "Servicio Satélite de Escaneo — TexCore"
Cohesion: 0.16
Nodes (6): 1. ¿Qué hace usted en TexCore?, 2. Ingresar al sistema, 3. Pestaña Fórmulas Químicas, 4. Pestaña Stock Disponible, 5. Preguntas frecuentes, Manual de Usuario — Tintorero

### Community 143 - "31 de Agosto de 2026 (continuación — tras ampliar recursos de la VM)"
Cohesion: 0.14
...
### Community 145 - "Hotspots Detallados"
Cohesion: 0.43
Nodes (4): _crear_lote(), _crear_orden(), _fixtures(), Tests F1: Modelo EventoEtiqueta + snapshot ORIGINAL en registro de lote. Artefac

### Community 146 - "Diagramas de Secuencia — Usuarios Principales del Sistema TexCore"
Cohesion: 0.06
Nodes (18): Command, BaseCommand, AnulacionPedidoSerializer, ModificacionPedidoSerializer, Revierte un pago deshaciendo el abono registrado.          Validaciones:, PagoReversionTestCase, TransactionTestCase, Caso 2: Reversión sin justificación falla          Verifica:         - ValueErro (+10 more)

### Community 147 - "Auditoría y Corrección RBAC — Roles Bodeguero / Despacho (módulo `inventory/`)"
Cohesion: 0.33
...
### Community 149 - "DespachoDashboard.test.tsx"
Cohesion: 0.13
...
### Community 150 - "carousel.tsx"
Cohesion: 0.22
Nodes (3): TestFechaATexto, TestFechaATexto, _fecha_a_texto()

### Community 151 - "VendedorDashboard.cobranza.test.tsx"
Cohesion: 0.13
...
### Community 154 - "FrontendLogViewTestCase"
Cohesion: 0.14
...
### Community 155 - "ProcessDespachoAPIViewTestCase"
Cohesion: 0.14
...
### Community 156 - "TestJWTTokenManager"
Cohesion: 0.14
...
### Community 157 - "reporting_proxy.py"
Cohesion: 0.14
Nodes (9): mockBodegas, mockDelete, mockGet, mockPost, mockProductos, mockProveedores, mockPut, toastErrorMock (+1 more)

### Community 158 - "Análisis del Rol de Vendedor - Sistema TexCore"
Cohesion: 0.07
Nodes (39): InventoryDashboardProps, calcularSaldoAcumulado(), normalizeBodegaKey(), StockItem, TransferFormData, validateTransfer(), KardexView, KardexViewProps (+31 more)

### Community 159 - "AUDITORÍA DE CALIDAD DE SOFTWARE — TEXCORE"
Cohesion: 0.16
Nodes (12): enterDespachoMode(), mockGet, mockNavigate, mockPedidosResponse(), mockPost, PEDIDO_1, PEDIDO_2, PEDIDO_3 (+4 more)

### Community 160 - "InventoryDashboard.reportes.test.tsx"
Cohesion: 0.20
Nodes (13): Carousel(), CarouselApi, CarouselContent(), CarouselContext, CarouselContextProps, CarouselItem(), CarouselNext(), CarouselOptions (+5 more)

### Community 161 - "EjecutivosDashboard.produccion-por-producto.test.tsx"
Cohesion: 0.18
Nodes (7): abrirExpedienteCliente(), abrirVentaNueva(), CLIENTE_1, esperarDirectorio(), irAReportes(), PEDIDO_1, PRODUCTO_1

### Community 162 - "ComponenteMezclaPanel.test.tsx"
Cohesion: 0.25
...
### Community 163 - "ManageLineas.test.tsx"
Cohesion: 0.08
Nodes (36): COLORS, Props, ESTADO_BADGE, ManageLineasProps, ESTADO_BADGE, ManageMaquinasProps, mockGet, NIVEL_CON_PASOS_Y_SIGUIENTE (+28 more)

### Community 164 - "EtapasProduccion.test.tsx"
Cohesion: 0.14
Nodes (3): FrontendLogViewTestCase, TestCase, Pruebas de las vistas de sistema — gestion/views/system_views.py.  FrontendLogVi

### Community 165 - "RegistrarTransformacion.test.tsx"
Cohesion: 0.27
...
### Community 166 - "ServiceCredential"
Cohesion: 0.40
...
### Community 167 - "ReportingProxyViewExtraTestCase"
Cohesion: 0.15
...
### Community 168 - "TestPdfReporteEndpoints"
Cohesion: 0.15
...
### Community 169 - "_prepare_df_for_excel"
Cohesion: 0.15
Nodes (12): Cómo ejecutar la suite, Defectos detectados y corregidos durante el refuerzo, Estado de cobertura, Fase 6 — Limpieza de `gestion/tests_integrados.py` (2026-09-02), Frontend (React), Leyenda de técnicas ISTQB, Matriz, Matriz de Trazabilidad de Pruebas — TexCore (Backend) (+4 more)

### Community 170 - "CONTEXTO — Análisis Exhaustivo de TexCore para Revisión Fable 5"
Cohesion: 0.04
Nodes (48): 10. RENDIMIENTO Y BASE DE DATOS, 11. MATRIZ DE HALLAZGOS, 12. PLAN DE ACCIÓN, 13. MÉTRICAS DE DEUDA TÉCNICA, 1. RESUMEN EJECUTIVO, 2. ALCANCE Y METODOLOGÍA, 6. CONSISTENCIA DEL PROYECTO, 8. GOBIERNO DE TI — COBIT 2019 (+40 more)
...
### Community 172 - "Guía de Comandos para Operaciones en Producción"
Cohesion: 0.17
...
### Community 176 - "Interfaz de Usuario (Frontend) - Sistema TexCore"
Cohesion: 0.14
Nodes (6): _make_refresh_token(), TestCase, Pruebas de internal_api/views/auth_views.py — ServiceTokenRefreshView y la rama, ServiceTokenRefreshViewTestCase, ServiceTokenRefreshViewThrottleTestCase, ServiceTokenViewValidationTestCase

### Community 177 - "TransformationView.test.tsx"
Cohesion: 0.17
...
### Community 178 - "BodegueroDashboard.test.tsx"
Cohesion: 0.17
...
### Community 180 - "TexCore — Configuración CI/CD con GitHub Actions"
Cohesion: 0.17
...
### Community 182 - "HistorialDespachosDocumentosTestCase"
Cohesion: 0.17
...
### Community 184 - "TexCore — Sistema Integral de Gestión para la Industria Textil"
Cohesion: 0.15
Nodes (10): ComponenteMezclaPanel(), BODEGA, COMPONENTE_1, mockDelete, mockGet, mockPost, PRODUCTO, SelectCtx (+2 more)

### Community 185 - "Grafo de Conocimiento y Navegación del Código (Graphify)"
Cohesion: 0.40
...
### Community 186 - "🛠️ Implementación Sprint 1 (Fable 5 — 10 Junio 2026) — TDD"
Cohesion: 0.14
Nodes (21): EmpaquetadoDashboard(), PackagingFormValues, packagingSchema, toLocalDatetimeInput(), AlertasInventarioPanel, AlertasInventarioPanelProps, Alert(), AlertDescription() (+13 more)

### Community 187 - "2. Scripts SQL de Eliminación Directa"
Cohesion: 0.40
Nodes (4): GenealogiaLoteModal(), DATA_ADELANTE, DATA_ATRAS, mockGet

### Community 188 - "Servicio Satélite: scanning_service"
Cohesion: 0.17
...
### Community 193 - "ManageMaquinas.test.tsx"
Cohesion: 0.18
...
### Community 194 - "InventoryForm.test.tsx"
Cohesion: 0.29
...
### Community 195 - "FormulaQuimica.test.tsx"
Cohesion: 0.18
...
### Community 197 - "UnifiedBusinessLogicTestCase"
Cohesion: 0.18
...
### Community 198 - "TestServiceTokenView"
Cohesion: 0.18
...
### Community 199 - "TestJWTServiceAuthentication"
Cohesion: 0.22
...
### Community 201 - "stress_test_data.py"
Cohesion: 0.18
...
### Community 209 - "TintoreroDashboard.test.tsx"
Cohesion: 0.12
...
### Community 210 - "navigation-menu.tsx"
Cohesion: 0.24
...
### Community 211 - "PedidoVentaOptimizationTest"
Cohesion: 0.20
Nodes (7): OutputFormatter, DataFrame, Protocol, Response, Protocol para formateadores de salida (Strategy Pattern). OCP: agregar formato P, Convierte un DataFrame en una Response HTTP descargable., ReportService: formatea los datos de un reporte a Excel/CSV. SRP: solo coordina

### Community 212 - "TestReportingEndpoints"
Cohesion: 0.10
...
### Community 213 - "PrecisionDecimalInventoryTestCase"
Cohesion: 0.12
...
### Community 217 - "3. Servicios y Componentes"
Cohesion: 0.20
...
### Community 224 - "ManageProductos.test.tsx"
Cohesion: 0.20
...
### Community 226 - "ReetiquetarModal.test.tsx"
Cohesion: 0.22
Nodes (3): _make_service_token(), TestCase, TestReportingEndpoints

### Community 228 - "SharedKPIChart.tsx"
Cohesion: 0.22
...
### Community 229 - "VendedorDashboard.anulacion.test.tsx"
Cohesion: 0.22
...
### Community 230 - "VendedorDashboard.cliente.test.tsx"
Cohesion: 0.17
...
### Community 231 - "SedeFilteringTestCase"
Cohesion: 0.22
...
### Community 232 - "ClienteImprovementsTestCase"
Cohesion: 0.22
...
### Community 233 - "PrintingServiceGuiaRemisionPdfTestCase"
Cohesion: 0.20
...
### Community 235 - "TestValidateLoteView"
Cohesion: 0.22
...
### Community 236 - "DespachReversionAPITestCase"
Cohesion: 0.22
...
### Community 237 - "HistorialDespachoUnitTests"
Cohesion: 0.22
...
### Community 240 - "🛠️ Implementación Sprint 6 (Fable 5 — 10 Junio 2026) — F0-001 + F0-002 + Protocolo 3-Fase"
Cohesion: 0.22
...
### Community 241 - "Funcionalidades Faltantes para Industria Textil"
Cohesion: 0.22
...
### Community 242 - "Documento de Contexto y Decisión Arquitectónica: TexCore — Sistema de gestión de órdenes de producción"
Cohesion: 0.22
Nodes (5): PRODUCTO_1, PRODUCTO_2, PRODUCTO_MERMA, SelectCtx, toastErrorMock

### Community 243 - "10. Decisiones de Arquitectura (ADRs)"
Cohesion: 0.22
Nodes (8): EVENTO_ORIGINAL, EVENTO_REETIQUETADO, mockGet, mockPost, SelectCtx, toastErrorMock, toastSuccessMock, writeTextMock

### Community 245 - "✅ Funcionalidades Implementadas"
Cohesion: 0.22
Nodes (8): mockGet, mockPost, ORDEN_1, ORDEN_2, SelectCtx, toastErrorMock, toastSuccessMock, TRANSFERENCIA_1

### Community 246 - "TexCore — Índice de Documentación"
Cohesion: 0.25
Nodes (7): ChartConfig, ChartType, DEFAULT_COLORS, SharedKPIChart(), SharedKPIChartProps, CONFIG, DATA

### Community 247 - "7. TESTING — ESTÁNDAR ISTQB"
Cohesion: 0.16
Nodes (9): Decimal, Punto de entrada único — Fachada sobre los métodos privados., Endpoint dedicado para la tendencia de 30 días (usado independientemente)., Serie diaria de kg producidos de UN producto — gráfica de drill-down (CU-EJ-09)., Genera la serie temporal completa (incluyendo días sin producción = 0)         p, Serie diaria de kg producidos, acotada a un único producto de salida., Indexa `rows` por fecha y rellena los días sin producción con kg=0., Un punto de la serie temporal de producción diaria. (+1 more)

### Community 251 - "PagoReversionAPITestCase"
Cohesion: 0.11
...
### Community 253 - ".format"
Cohesion: 0.22
Nodes (3): HttpUser, Prueba de carga concurrente TexCore — simula ~100 usuarios reales.  Uso:     loc, UsuarioTexCore

### Community 256 - "coverage_html_cb_6fb7b396.js"
Cohesion: 0.22
Nodes (7): LOTE, mockPost, mockPrintLabel, mockRole, SelectCtx, toastErrorMock, toastSuccessMock

### Community 257 - "Febrero 2026"
Cohesion: 0.25
...
### Community 258 - "Marzo 2026"
Cohesion: 0.09
Nodes (23): 17 de Septiembre de 2026, 1. Inventario y Stock Comprometido (Fase 1 Frontend — COMPLETADA 100%), 2 de Septiembre de 2026, 2. Manufactura Continua y Reestructuración de Dashboards (Fase 2 Frontend — COMPLETADA 100%), 3 de Septiembre de 2026, 3. Planificación Contra Stock MTS (Fase 3 Frontend — COMPLETADA 100%), 4. Producción Bajo Pedido MTO (Fase 4 Frontend — COMPLETADA 100%), 5. Estado Actual del Plan y Próximos Pasos (Fase 5 y 6) (+15 more)

### Community 259 - "Análisis de Errores Críticos (P0)"
Cohesion: 0.25
...
### Community 260 - "9. Testing y Calidad"
Cohesion: 0.25
...
### Community 261 - "10.2 Configurar Backups Automáticos"
Cohesion: 0.40
Nodes (4): CORRIDA_ACTIVA, mockGet, mockPost, OPERACIONES

### Community 262 - "🐛 Troubleshooting"
Cohesion: 0.25
...
### Community 263 - "5. CALIDAD DE CÓDIGO — PRINCIPIOS SOLID Y DRY"
Cohesion: 0.25
...
### Community 265 - "ManageQuimicos.test.tsx"
Cohesion: 0.25
...
### Community 266 - "FlujoProduccion.test.tsx"
Cohesion: 0.25
...
### Community 267 - "toggle-group.tsx"
Cohesion: 0.25
...
### Community 268 - "Cliente"
Cohesion: 0.25
Nodes (7): background_color, display, icons, name, short_name, start_url, theme_color

### Community 269 - "errorUtils.ts"
Cohesion: 0.25
Nodes (6): mockDelete, mockGet, mockPatch, mockPost, toastErrorMock, toastSuccessMock

### Community 271 - "TestDocumentService_FormatearFecha"
Cohesion: 0.25
Nodes (3): CreateAdminCommandTestCase, TestCase, Pruebas de gestion/management/commands/create_admin.py — Fase 1.1 del barrido de

### Community 274 - "Prueba de carga — 100 usuarios concurrentes"
Cohesion: 0.32
Nodes (3): getCellValue(), rowComparator(), sortColumn()

### Community 276 - "4. Autenticacion y Seguridad"
Cohesion: 0.29
...
### Community 277 - "🗄️ Paso 6: Inicializar la Base de Datos"
Cohesion: 0.29
...
### Community 278 - "7.2 Desde Otra Máquina en la Red"
Cohesion: 0.29
...
### Community 279 - "Plan de Evolución Industrial - TexCore MES/ERP"
Cohesion: 0.29
...
### Community 281 - "Django Internal API (/api/internal/v1/)"
Cohesion: 0.29
...
### Community 282 - "ProcessDespachoAPIView"
Cohesion: 0.29
...
### Community 283 - "13. MÉTRICAS DE DEUDA TÉCNICA"
Cohesion: 0.29
Nodes (4): App(), mockAuthState, queryClient, root

### Community 284 - "KardexView.test.tsx"
Cohesion: 0.29
Nodes (4): QUIMICO_1, QUIMICO_2, SelectCtx, toastErrorMock

### Community 285 - "ReportesView.test.tsx"
Cohesion: 0.29
Nodes (5): ETAPA, mockGet, ORDEN_CON_ETAPAS, ORDEN_SIN_ETAPAS, toastErrorMock

### Community 286 - "EliminarMovimientoDialog.test.tsx"
Cohesion: 0.38
Nodes (5): ToggleGroup, ToggleGroupContext, ToggleGroupItem, Toggle(), toggleVariants

### Community 287 - "GuiaRemisionModal.test.tsx"
Cohesion: 0.33
...
### Community 288 - "BuscadorLotes.test.tsx"
Cohesion: 0.43
...
### Community 291 - "JefeAreaLogicTest"
Cohesion: 0.07
Nodes (23): CorridaProduccion, DetallePlanProduccion, _crear_movimiento_inventario(), Decimal, OperacionProduccion, Revierte atómicamente una operación de producción:         - Verifica que los lo, Registra un paso unitario de transformación en máquina dentro de una corrida., DetallePlanProduccionFactory (+15 more)

### Community 295 - "TexCore — Atribuciones y Licencias de Terceros"
Cohesion: 0.29
Nodes (6): Cómo leer los resultados, Instalación (una sola vez), Notas, Prueba de carga — 100 usuarios concurrentes, Secuencia recomendada, Umbrales de referencia ("sigue siendo rápido")

### Community 296 - "19 de Agosto de 2026"
Cohesion: 0.06
...
### Community 297 - "Plan de Acción Prioritario"
Cohesion: 0.33
...
### Community 298 - "Problemas de Degradación (P1)"
Cohesion: 0.33
...
### Community 299 - "1. Vision General del Sistema"
Cohesion: 0.33
...
### Community 300 - "2. Diagrama de Arquitectura C4"
Cohesion: 0.33
...
### Community 301 - "5. Modelos de Datos"
Cohesion: 0.33
...
### Community 302 - "7. Flujos de Negocio Criticos"
Cohesion: 0.33
...
### Community 303 - "TexCore — Arquitectura del Sistema"
Cohesion: 0.33
...
### Community 305 - "🔧 Paso 1: Instalación de Dependencias Base"
Cohesion: 0.33
Nodes (4): mockUseKardex, MOV_ENTRADA, MOV_SALIDA, SelectCtx

### Community 306 - "Autenticacion JWT RS256 entre Servicios"
Cohesion: 0.33
Nodes (4): BODEGA, mockHandleExport, PRODUCTOS, SelectCtx

### Community 307 - "Scripts y Comandos de Gestion"
Cohesion: 0.33
Nodes (4): mockDelete, MOVIMIENTO, toastErrorMock, toastSuccessMock

### Community 308 - "10. RENDIMIENTO Y BASE DE DATOS"
Cohesion: 0.40
Nodes (5): completarCamposObligatorios(), fireEventChange(), mockPost, toastErrorMock, toastSuccessMock

### Community 309 - "6. CONSISTENCIA DEL PROYECTO"
Cohesion: 0.33
Nodes (4): mockGet, mockRole, SelectCtx, toastErrorMock

### Community 310 - "8. GOBIERNO DE TI — COBIT 2019"
Cohesion: 0.33
Nodes (4): LOG_CREATE, LOG_DELETE, LOG_UPDATE, mockGet

### Community 316 - "verificar_auditoria.py"
Cohesion: 0.13
Nodes (19): _audit(), BalanceMasasPdfView, _build_reporte_avance_payload(), _get_printing_url(), _now_iso(), _proxy_pdf(), APIView, Request (+11 more)

### Community 317 - "Módulo de Gestión (Core Business)"
Cohesion: 0.33
Nodes (5): bypass_jwt(), mock_db_connection(), conftest.py del reporting_excel. Proporciona fixtures compartidos para tests., Reemplaza jwt.decode por un mock que acepta cualquier Bearer token en tests., No-op: fixture mantenido por compatibilidad de firma con tests existentes     qu

### Community 321 - ".format"
Cohesion: 0.60
Nodes (5): print_cyan(), print_error(), print_green(), print_yellow(), deploy_prod.sh script

### Community 322 - "_RealDomainRepo"
Cohesion: 0.40
Nodes (4): Backend & Servicios Satélite, Frontend, Recursos de Diseño y Medios, TexCore — Atribuciones y Licencias de Terceros

### Community 323 - "deploy.sh script"
Cohesion: 0.12
...
### Community 324 - "Scripts de Utilidad"
Cohesion: 0.40
...
### Community 326 - "Problemas de Arquitectura y Patrones"
Cohesion: 0.40
...
### Community 327 - "Estado del Proyecto"
Cohesion: 0.40
...
### Community 328 - "Resumen Ejecutivo"
Cohesion: 0.40
...
### Community 329 - "Docker — Configuración específica por plataforma"
Cohesion: 0.40
...
### Community 330 - "⚙️ Paso 3: Configuración de Variables de Entorno"
Cohesion: 0.40
...
### Community 331 - "🔐 Paso 4: Configurar Certificados SSL"
Cohesion: 0.40
...
### Community 333 - "✅ Paso 8: Verificación del Deployment"
Cohesion: 0.40
...
### Community 334 - "🔄 Paso 9: Gestión del Deployment"
Cohesion: 0.40
...
### Community 359 - "TestPdfProduccionViews"
Cohesion: 0.50
...
### Community 362 - "@hookform/resolvers"
Cohesion: 0.50
...
### Community 366 - "@radix-ui/react-aspect-ratio"
Cohesion: 0.50
Nodes (3): Barrido de Higiene — Fase 2: Código Muerto Confirmado, Verificación de cierre, Ítems eliminados

### Community 374 - "@radix-ui/react-scroll-area"
Cohesion: 0.50
Nodes (3): Migration, precargar_configuracion_por_sede_existente(), Crea una ConfiguracionEmpaqueSede (15 fundas/baño, 15 conos/funda — los     valo

### Community 376 - "@radix-ui/react-separator"
Cohesion: 0.50
Nodes (3): Módulo Internal API, Responsabilidades, Seguridad

### Community 377 - "@radix-ui/react-slider"
Cohesion: 0.67
...
### Community 378 - "@radix-ui/react-switch"
Cohesion: 0.67
Nodes (3): Regresión de concurrencia del microservicio completo.  El Dockerfile levanta rep, _slow_generate(), test_export_dado_peticiones_concurrentes_cuando_formateo_es_lento_entonces_no_se_serializan()

### Community 380 - "react-day-picker"
Cohesion: 0.67
...
### Community 404 - "Fase 4 — `gestion/models.py` (1655 líneas → 7 archivos, paquete nuevo)"
Cohesion: 0.40
...
### Community 406 - "GenealogiaService"
Cohesion: 0.07
Nodes (43): CorridaProduccionSerializer, CrearPlanDesdeAlertasInputSerializer, DetallePlanProduccionSerializer, GenerarOrdenDesdePlanInputSerializer, IniciarCorridaInputSerializer, OperacionProduccionSerializer, PlanProduccionSerializer, RegistroOperacionInputSerializer (+35 more)

### Community 410 - ".test_new_credit_terms_due_date_calculation"
Cohesion: 0.20
Nodes (7): Command, BaseCommand, Registra las credenciales de los microservicios en la BD (ServiceCredential). De, Meta, ServiceCredential: identidad de un microservicio autorizado. ISO 27001 A.9.2 — G, Representa la identidad de un microservicio que consume la API interna., ServiceCredential

### Community 412 - ".test_block_cash_payment_no_payment_second_order"
Cohesion: 0.21
...
### Community 416 - ".test_salesman_filtering"
Cohesion: 0.22
...
### Community 418 - "Fase 1 — `gestion/views/production_views.py` (1766 líneas → 5 archivos + 1 común)"
Cohesion: 0.40
Nodes (5): Archivos existentes a editar, Archivos nuevos a crear (todos en `gestion/views/`), Catálogo actual (validado línea por línea), Fase 1 — `gestion/views/production_views.py` (1766 líneas → 5 archivos + 1 común), Sin cambios necesarios

### Community 542 - "CustomUserAdmin"
Cohesion: 0.10
Nodes (14): Pobla clientes, pedidos de venta y pagos para probar el dashboard de Ventas (eje, Cliente, DetallePedido, PagoCliente, PedidoVenta, Meta, Servicio de Reversión de Pagos del Cliente Artefacto RUP: Módulo de Servicio Cas, Tests Sprint 2: Anticipos de Cliente (P1-002) y Pagos Parciales (P1-003) Artefac (+6 more)

## Knowledge Gaps
- **2073 isolated node(s):** `name`, `version`, `private`, `@hookform/resolvers`, `@radix-ui/react-accordion` (+2068 more)
...
- **Why does `TestFechaATexto` connect `carousel.tsx` to `ExecutiveKPIService`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `ExcelFormatter` connect `ExecutiveKPIService` to `JefeAreaDashboard.test.tsx`, `carousel.tsx`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Are the 119 inferred relationships involving `CustomUserFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`CustomUserFactory` has 119 INFERRED edges - model-reasoned connections that need verification._
- **Are the 112 inferred relationships involving `SedeFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`SedeFactory` has 112 INFERRED edges - model-reasoned connections that need verification._
- **Are the 93 inferred relationships involving `ProductoFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`ProductoFactory` has 93 INFERRED edges - model-reasoned connections that need verification._
- **Are the 69 inferred relationships involving `OrdenProduccionFactory` (e.g. with `ConfiguracionEmpaqueSedeModelTestCase` and `LoteProduccionPresentacionEmpaqueTestCase`) actually correct?**
  _`OrdenProduccionFactory` has 69 INFERRED edges - model-reasoned connections that need verification._
- **What connects `ASGI config for TexCore project.  It exposes the ASGI callable as a module-level`, `RFC 5424 Syslog Formatter — TexCore Backend (Django) ===========================`, `Formatter RFC 5424 para Django/Python logging.      Produce una línea por regist` to the rest of the system?**
  _3026 weakly-connected nodes found - possible documentation gaps or missing edges._