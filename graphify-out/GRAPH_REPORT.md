# Graph Report - TexCore  (2026-09-15)

## Corpus Check
- 730 files · ~1,058,557 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7906 nodes · 17044 edges · 521 communities (408 shown, 113 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 1952 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3e4a17c3`
...
1. `CustomUserFactory` - 422 edges
2. `SedeFactory` - 241 edges
3. `cn()` - 221 edges
4. `ProductoFactory` - 183 edges
5. `OrdenProduccionFactory` - 148 edges
6. `AreaFactory` - 132 edges
7. `StockBodega` - 104 edges
8. `BodegaFactory` - 99 edges
9. `MaquinaFactory` - 99 edges
10. `LoteProduccionFactory` - 98 edges
...
## Communities (521 total, 113 thin omitted)

### Community 0 - "Producto"
Cohesion: 0.07
Nodes (28): _apply_pragmas(), Base, get_session_factory(), _make_engine(), _make_session_factory(), async_sessionmaker, AsyncSession, DeclarativeBase (+20 more)

### Community 1 - "StockBodega"
Cohesion: 0.06
Nodes (34): calcularSaldoAcumulado(), normalizeBodegaKey(), StockItem, TransferFormData, validateTransfer(), KardexViewImpl(), ReportesViewImpl(), StockView (+26 more)

### Community 2 - "CustomUserFactory"
Cohesion: 0.03
Nodes (32): CustomUserFactory, AreaViewSetTestCase, CustomUserViewSetTestCase, TestCase, SedeViewSetTestCase, CustomTokenObtainPairViewTestCase, CustomTokenRefreshViewTestCase, LogoutViewTestCase (+24 more)

### Community 3 - "types.ts"
Cohesion: 0.07
Nodes (49): AbstractUser, Seed de SIMULACIÓN INTEGRAL de TexCore.  Puebla la base con un caso coherente en, Meta, Area, AuditableModelMixin, AuditLog, ConfiguracionEmpaqueSede, CustomUser (+41 more)

### Community 4 - "LoteProduccionFactory"
Cohesion: 0.05
Nodes (29): make_group_permission(), Permisos de acceso para TexCore.  Usa make_group_permission() para eliminar la d, Factory para crear clases de permiso basadas en grupos de Django.      Otorga ac, PagoReversionService, Servicio de reversión de pagos.     Responsable único: deshacer un pago y restau, PaymentReconciler, Utilidad para reconciliar los pagos de clientes contra sus pedidos (FIFO).     A, ChemicalViewSet (+21 more)
...
Cohesion: 0.05
Nodes (24): Si la máquina del lote tiene producto_merma configurado y         peso_merma > 0, Revierte el stock de merma creado por este lote., LoteProduccionFactory, MaquinaConMermaFactory, ConfiguracionEmpaqueSedeModelTestCase, LoteProduccionPresentacionEmpaqueTestCase, MRPEngineConfiguracionEmpaqueTestCase, TestCase (+16 more)

### Community 8 - "BodegaFactory"
Cohesion: 0.02
Nodes (120): AdminSistemasDashboard(), InventoryDashboard(), InventoryDashboardProps, KardexView, KardexViewProps, ManageAreas(), ManageAreasProps, AREA_1 (+112 more)

### Community 9 - "EjecutivosDashboard.tsx"
Cohesion: 0.07
Nodes (32): RegistrarLoteProduccionSerializer, ConsumoMezclaService, SRP: gestiona el consumo de múltiples lotes de entrada para producir un lote de, EventoEtiquetaService, EventoEtiquetaService: orquesta la numeración y creación de eventos de etiqueta., LoteStockAdjustmentService, Decimal, Reversión manual de stock al rechazar un lote: salida producida,         materia (+24 more)

### Community 10 - "JefeAreaDashboard.tsx"
Cohesion: 0.03
Nodes (31): ClienteFactory, ClienteAuditoriaJustificacionTestCase, TestCase, Pruebas de ClienteViewSet — requisito de justificación de auditoría en updates., TestCase, Tests de Cliente — límite de crédito. Aplica técnicas ISTQB:   - Partición de Eq, ISTQB EP — Clases de equivalencia para limite_credito:       Válida:   limite_cr, BVA: valor mínimo exacto del límite. (+23 more)

### Community 11 - "card.tsx"
Cohesion: 0.03
Nodes (36): ParoMaquina, Registro de downtime de máquina, con reason code = las Seis Grandes Pérdidas, AreaFactory, MaquinaFactory, ParoMaquinaFactory, SedeFactory, Pruebas de gestion/views/core_views.py — SedeViewSet, AreaViewSet, CustomUserVie, KPIAreaCalidadRendimientoTestCase (+28 more)

### Community 12 - "AreaFactory"
Cohesion: 0.04
Nodes (24): EventoEtiquetaFactory, OrdenProduccionFactory, TestCase, Tests de OrdenProduccion — máquina de estados. Aplica técnica ISTQB: State Trans, ISTQB STT — Transiciones legales en la máquina de estados., El modelo actual permite pendiente → finalizada (no tiene validación de estado)., Tests de creación de OrdenProduccion con validaciones de campos., BVA: peso_neto_requerido = 0 debería ser inválido para una orden de producción. (+16 more)
...
Cohesion: 0.11
Nodes (57): StockItem, EditarMovimientoDialog(), EditarMovimientoDialogProps, EliminarMovimientoDialog(), EliminarMovimientoDialogProps, RegistrarMermaDialog(), GuiaRemisionModalProps, MOTIVOS_TRASLADO (+49 more)

### Community 15 - "ManageOrdenesProduccion.tsx"
Cohesion: 0.05
Nodes (16): CustomUserSerializer, Ejecutivos tienen acceso a todo el dashboard de stock: asignar todas las bodegas, DetalleFormulaSerializer, ComponenteMezclaOPSerializer, OrdenProduccionEstadoSerializer, DetallePedidoSerializer, ClienteSerializerValidateTieneBeneficioTestCase, ComponenteMezclaOPSerializerTestCase (+8 more)
...
Cohesion: 0.04
Nodes (41): Meta, ProductoSerializer, ProveedorSerializer, GroupSerializer, Meta, SedeSerializer, DetalleFormulaEscrituraSerializer, FaseRecetaEscrituraSerializer (+33 more)
...
Cohesion: 0.07
Nodes (61): AdminSedeDashboard(), ManageClientesProps, ClienteComprasModal(), ClienteComprasModalProps, ClienteDeudorModal(), ClienteDeudorModalProps, DeudorExtendido, PedidosEstadoModal() (+53 more)

### Community 22 - "__init__.py"
Cohesion: 0.04
Nodes (76): Elimina toda la población de datos creada para pruebas (stress_test_data y seed_, Artefacto RUP: Módulo de Servicio Caso de Uso: CU-DescargaQuimicaAutomatica Patr, DetalleHistorialDespachoAdmin, DetalleHistorialDespachoInline, HistorialDespachoAdmin, MovimientoInventarioAdmin, StockBodegaAdmin, AuditoriaMovimiento (+68 more)

### Community 23 - "ProduccionKPIService"
Cohesion: 0.06
Nodes (32): date, OpsEstado, ProduccionKPIs, ProduccionKPIService, ProduccionProductoItem, Decimal, RUP - Capa de Servicio: ProduccionKPIService ===================================, Punto de entrada único — Fachada sobre los métodos privados. (+24 more)

### Community 24 - "SedeFactory"
Cohesion: 0.06
Nodes (44): AccordionContent(), AccordionItem(), AccordionTrigger(), AlertDialogOverlay(), Avatar(), AvatarFallback(), AvatarImage(), BreadcrumbEllipsis() (+36 more)
...
Cohesion: 0.07
Nodes (73): ApprovalRequests(), ManageBodegas(), ManageFormulas(), ManageProductos(), ManageProveedores(), ManageQuimicos(), ManageQuimicosProps, Group (+65 more)
...
Cohesion: 0.06
Nodes (35): AnularPedidoModal, ClienteDetailDialog, ClienteDetailDialogImpl(), ClienteDetailDialogProps, EditarPedidoModal, mockPatch, PEDIDO, toastErrorMock (+27 more)

### Community 29 - "OperarioDashboard.tsx"
Cohesion: 0.07
Nodes (27): APIView, API para registrar la TRANSFORMACIÓN de un producto en otro (ej. Proceso product, TransformacionAPIView, AuditLogPagination, AuditLogViewSet, PageNumberPagination, KardexBodegaAPIView, MovimientosPorLoteAPIView (+19 more)
...
### Community 31 - "pdf.py"
Cohesion: 0.10
Nodes (19): CostoLoteService, Tarifa vigente a la fecha; vigente_hasta NULL = contrato abierto., Costo/hora vigente a la fecha; vigente_hasta NULL = sin fecha fin., Calcula y persiste el desglose de costos de un lote de producción., CosteoCalculoTestCase, CosteoEndpointTestCase, CosteoLoteBaseTestCase, TestCase (+11 more)
...
Cohesion: 0.07
Nodes (51): Configuración centralizada del printing_service. Elimina dependencias de cwd (fr, build_print_record(), get_audit_repo(), AuditRepository de printing_service: persiste eventos de auditoría en SQLite. DI, Factory function — construye PrintAuditLog. SRP: separa construcción de persiste, Dependency provider para FastAPI Depends. DIP: el router no construye el repo., generate_balance_masas_pdf(), generate_etiqueta_pdf() (+43 more)

### Community 38 - "Documentación Técnica: Sistema de Reversión de Pagos del Cliente"
Cohesion: 0.05
Nodes (39): class-variance-authority, embla-carousel-react, dependencies, class-variance-authority, embla-carousel-react, input-otp, jwt-decode, @radix-ui/react-alert-dialog (+31 more)

### Community 39 - "__init__.py"
Cohesion: 0.04
Nodes (31): BodegaFactory, StockBodegaFactory, TestCase, Fase 1.5b del barrido de higiene (2026-09-01): generate_next_lote_codigo() (gest, RegistroLoteLockTestCase, get_stock_bajo(), GetStockBajoTestCase, TestCase (+23 more)
...
Cohesion: 0.10
Nodes (9): Command, BaseCommand, AnulacionPedidoSerializer, ModificacionPedidoSerializer, DetallePedidoViewSet, P0-005: pago + reconciliación en una sola transacción, con lock         pesimist, Anula un pedido en estado 'pendiente'.         Requiere motivo_anulacion (mínimo, Modifica campos de un pedido en estado 'pendiente' y no anulado.         Requier (+1 more)
...
Cohesion: 0.05
Nodes (43): Proveedor, ConsumoMateriaPrima, MateriaPrimaLote, Through-table: LoteProduccion <-> MateriaPrimaLote (inmutable post-registro), MateriaPrimaLoteSerializer, Meta, Entrada del endpoint registrar_entrada — la creación real la hace el servicio., RegistrarMateriaPrimaSerializer (+35 more)
...
Cohesion: 0.17
Nodes (10): KardexParams, BaseModel, Schemas Pydantic para parámetros de los reportes. ISP: un schema por caso de uso, Parámetros para reportes por vendedor., StockParams, VendedorParams, Tests unitarios para src/schemas/report_params.py. EP + BVA sobre validadores Py, TestKardexParams (+2 more)
...
Cohesion: 0.04
Nodes (33): Continuidad de cadena: entrada = salida de la transformación previa., Servicio de aplicación para registrar transformaciones de producto., Registra una transformación en la OP.          El ``producto_entrada`` NO se con, TransformacionService, Meta, ProductoFactory, ProveedorFactory, Factories para tests de TexCore — usa factory_boy.  Convención: - Cada factory c (+25 more)
...
Cohesion: 0.12
Nodes (21): BuildBalanceMasasPayloadTestCase, Función pura: reconcilia stock actual + movimientos del mes por producto.     Cu, _audit(), BalanceMasasPdfView, _build_balance_masas_payload(), _get_printing_url(), _now_iso(), _proxy_pdf() (+13 more)
...
Cohesion: 0.10
Nodes (6): BuildReporteAvancePayloadTestCase, APITestCase, Función pura (sin ORM en su firma): mapea registros ya materializados     (dicts, TestPdfProduccionViews, _build_reporte_avance_payload(), Traduce los registros del ORM al schema ReporteAvanceRequest del     printing_se
...
Cohesion: 0.08
Nodes (26): 10 de Julio de 2026, 13 de Julio de 2026, 1 de Julio de 2026, 21 de Julio de 2026, 22 de Julio de 2026, 22 de Julio de 2026, 22 de Julio de 2026 (2), 22 de Julio de 2026 (3) (+18 more)
...
Cohesion: 0.06
Nodes (18): ParseIntParamTestCase, SimpleTestCase, Pruebas de gestion/views/_common.py — helpers compartidos entre módulos de vista, parse_int_param(), Valida un query param usado como ID entero positivo (OWASP A03).      Retorna in, ComponenteMezclaOPViewSet, ConsumoLoteDetalleViewSet, CRUD de componentes de mezcla.     ISO 27001 A.9.4: solo jefe_area o admin puede (+10 more)
...
Cohesion: 0.08
Nodes (30): clear_cascade_justification(), get_cascade_justification(), get_current_ip(), get_current_user(), Justificación para borrados en cascada (ej. DetalleFormula al borrar FormulaColo, set_cascade_justification(), _get_object_sede_id(), Validaciones de negocio que requieren contexto de auditoría.         Se llama au (+22 more)
...
Cohesion: 0.18
Nodes (6): DjangoApiClient, StockBodega, Retorna StockBodega desde caché poblado por get_lote_by_codigo., Adapter que implementa ILoteRepository haciendo UNA llamada HTTP a Django.     D, Tests para DjangoApiClient. EP + circuit breaker., TestDjangoApiClient

### Community 124 - "menubar.tsx"
Cohesion: 0.09
Nodes (20): react, ChartConfig, ChartContainer(), ChartContext, ChartContextProps, ChartLegendContent(), ChartTooltipContent(), getPayloadConfigFromPayload() (+12 more)
...
Cohesion: 0.08
Nodes (39): ESTADO_BADGE, ManageLineasProps, ESTADO_BADGE, ManageMaquinasProps, MaquinaCardInline, MaquinaCardInlineProps, MaquinaDialogProps, MaquinasPorLineaPanelImpl() (+31 more)
...
Cohesion: 0.15
Nodes (9): Command, BaseCommand, Genera hash seguro del secret usando el hasher de Django (PBKDF2/bcrypt)., TestCase, Tests TDD para ServiceCredential. EP + STT., STT: transición activo → inactivo revoca acceso., EP: creación normal de una credencial de servicio., TestServiceCredentialCreacion (+1 more)
...
Cohesion: 0.05
Nodes (24): EventoEtiqueta, LoteProduccionSerializer, LoteProduccion, Copia idéntica: mantiene la version de datos vigente, solo avanza la secuencia., Cambio de datos: anula la última etiqueta vigente y emite una nueva version., _snapshot_actual(), EventoEtiquetaModelTests, LoteProduccionSerializerValidateDesviacionOrdenTestCase (+16 more)
...
Cohesion: 0.06
Nodes (18): PrintingServiceGuiaRemisionPdfTestCase, PrintingServiceHistorialDespachosPdfTestCase, PrintingServiceLabelPdfTestCase, PrintingServiceNotaVentaPdfTestCase, PrintingServiceProduccionPorProductoPdfTestCase, PrintingServiceUrlResolutionTestCase, PrintingServiceZplTestCase, TestCase (+10 more)
...
### Community 148 - "InventoryDashboard.test.tsx"
Cohesion: 0.14
...
Cohesion: 0.14
Nodes (13): get_validation_service(), AuditRepository, BackgroundTasks, Request, Router HTTP para validación de lotes. DIP: get_validation_service y get_audit_re, Valida un código de lote escaneado (QR o código de barras).      - **code**: Cód, validate_lote(), BaseModel (+5 more)

### Community 152 - "printing.ts"
Cohesion: 0.22
Nodes (3): TestFechaATexto, TestFechaATexto, _fecha_a_texto()

### Community 153 - "OptionalPaginationTestCase"
Cohesion: 0.13
...
### Community 190 - "2. Plan de Mejora Arquitectónica (S.O.L.I.D. & Patrones)"
Cohesion: 0.17
...
Cohesion: 0.13
Nodes (21): AnonRateThrottle, Registra las credenciales de los microservicios en la BD (ServiceCredential). De, Meta, ServiceCredential: identidad de un microservicio autorizado. ISO 27001 A.9.2 — G, Representa la identidad de un microservicio que consume la API interna., ServiceCredential, Serializers para la API interna. ISP: un serializer por caso de uso., ServiceTokenRefreshRequestSerializer (+13 more)
...
Cohesion: 0.20
Nodes (9): DespachoEstadoService, DespachoEstadoService — determina el estado real de un PedidoVenta según lo efec, Suma, por producto_id, el peso de DetalleHistorialDespacho asignados a         e, Suma el peso requerido por producto_id a partir de los detalles del pedido., Determina qué estado debería tener el pedido AHORA MISMO según lo         realme, ProcessDespachoAPIView, APIView, Procesa el despacho de múltiples pedidos y lotes escaneados.     Descuenta inven (+1 more)
...
### Community 225 - "HistorialEtiquetasModal.test.tsx"
Cohesion: 0.20
...
### Community 227 - "TransferenciasInterarea.test.tsx"
Cohesion: 0.22
...
Cohesion: 0.09
Nodes (25): OeeService, Decimal, OeeService: calcula el Overall Equipment Effectiveness (OEE) por máquina o área., PrintingService, GroupViewSet, SedeViewSet, KPIAreaView, KpiEjecutivoView (+17 more)
...
### Community 244 - "8. Infraestructura y Despliegue"
Cohesion: 0.25
...
### Community 252 - "KardexFilterTests"
Cohesion: 0.20
...
### Community 254 - "RFC5424Formatter"
Cohesion: 0.25
...
### Community 255 - "dataframe_to_excel_bytes"
Cohesion: 0.29
...
### Community 270 - "CustomUserViewSet"
Cohesion: 0.38
Nodes (5): ToggleGroup, ToggleGroupContext, ToggleGroupItem, Toggle(), toggleVariants

### Community 272 - "CsvFormatter"
Cohesion: 0.25
...
### Community 275 - "5 de Junio de 2026"
Cohesion: 0.29
...
Cohesion: 0.05
Nodes (50): claseSeveridadEficiencia(), JefePlantaDashboard(), logger, toastErrorMock, toastSuccessMock, RFC-5424, UsuarioBasico, logger (+42 more)
...
### Community 289 - "AuditLogViewer.test.tsx"
Cohesion: 0.40
...
### Community 293 - "test_generate.py"
Cohesion: 0.33
...
### Community 294 - "deploy_prod.sh script"
Cohesion: 0.33
...
### Community 304 - "📊 Monitoreo y Mantenimiento"
Cohesion: 0.33
...
### Community 311 - "9. GESTIÓN DE SERVICIOS — ITIL 4"
Cohesion: 0.40
...
### Community 312 - "StockQuimicosDashboard.test.tsx"
Cohesion: 0.25
Nodes (5): ManageClientes(), CLIENTE_1, CLIENTE_2, SelectCtx, toastErrorMock

### Community 314 - "Command"
Cohesion: 0.60
Nodes (5): print_cyan(), print_error(), print_green(), print_yellow(), deploy_prod.sh script

### Community 315 - "seed_production_masters.py"
Cohesion: 0.40
...
### Community 319 - "Módulo de Inventario"
Cohesion: 0.40
...
Cohesion: 0.06
Nodes (42): BodegaAdmin, BodegueroInline, CustomUserAdmin, LineaProduccionAdmin, TransformacionProductoAdmin, Carga ~1,000,000 registros para estresar el aplicativo TexCore.  Usa bulk_create, Command, BaseCommand (+34 more)
...
### Community 347 - "Docker Compose (produccion)"
Cohesion: 0.40
...
### Community 368 - "@radix-ui/react-checkbox"
Cohesion: 0.50
Nodes (3): Módulo Internal API, Responsabilidades, Seguridad

### Community 369 - "@radix-ui/react-collapsible"
Cohesion: 0.67
...
### Community 370 - "@radix-ui/react-dropdown-menu"
Cohesion: 0.67
...
### Community 372 - "@radix-ui/react-progress"
Cohesion: 0.67
...
### Community 406 - ".test_dynamic_balance_calculation"
Cohesion: 0.25
...
### Community 407 - ".test_empaquetado_flujo_avanzado"
Cohesion: 0.40
...
### Community 408 - ".test_payment_tracking"
Cohesion: 0.40
...
Cohesion: 0.04
Nodes (59): AppContent(), getData(), AuditoriaDialog(), LOG_1, mockGet, BodegueroDashboard(), BuscadorLotes(), ROLES_SUPERVISOR (+51 more)
...
### Community 418 - "RegistrarParoModal.test.tsx"
Cohesion: 0.50
...
- **Why does `TestFechaATexto` connect `printing.ts` to `TransicionTresFasesTestCase`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `ExcelFormatter` connect `TransicionTresFasesTestCase` to `printing.ts`, `Plan: dividir los 4 archivos "dios" del backend Django (v2 — revalidado contra el código actual)`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Are the 113 inferred relationships involving `CustomUserFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`CustomUserFactory` has 113 INFERRED edges - model-reasoned connections that need verification._
- **Are the 105 inferred relationships involving `SedeFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`SedeFactory` has 105 INFERRED edges - model-reasoned connections that need verification._
- **Are the 86 inferred relationships involving `ProductoFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`ProductoFactory` has 86 INFERRED edges - model-reasoned connections that need verification._
- **Are the 65 inferred relationships involving `OrdenProduccionFactory` (e.g. with `ConfiguracionEmpaqueSedeModelTestCase` and `LoteProduccionPresentacionEmpaqueTestCase`) actually correct?**
  _`OrdenProduccionFactory` has 65 INFERRED edges - model-reasoned connections that need verification._
- **What connects `ASGI config for TexCore project.  It exposes the ASGI callable as a module-level`, `RFC 5424 Syslog Formatter — TexCore Backend (Django) ===========================`, `Formatter RFC 5424 para Django/Python logging.      Produce una línea por regist` to the rest of the system?**
  _2883 weakly-connected nodes found - possible documentation gaps or missing edges._