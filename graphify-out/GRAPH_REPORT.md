# Graph Report - TexCore  (2026-08-28)

## Corpus Check
- 712 files · ~508,360 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7650 nodes · 16587 edges · 468 communities (390 shown, 78 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 1823 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `29b2e2b9`
...
- JWTServiceAuthenticationExtraTestCase
- export_top_clientes
- scanning_views.py
...
- .reetiquetar
- ._build_zpl_fallback
- PrintingServiceNotaVentaPdfTestCase
- TestValidateLoteView
- TestSoloAscii
- .get
...
- .validate
- health.py
- TestValidateEndpoint_ValidacionPydantic
...
- .test_registrar_lote_empaquetado_completo
- .test_salesman_order_filtering
- .test_despacho_proceso_completo
- 20 de Julio de 2026
- 0001_initial.py
- 0001_initial.py
- __init__.py
...
1. `CustomUserFactory` - 388 edges
2. `cn()` - 221 edges
3. `SedeFactory` - 217 edges
4. `ProductoFactory` - 166 edges
5. `OrdenProduccionFactory` - 130 edges
6. `AreaFactory` - 124 edges
7. `StockBodega` - 101 edges
8. `MaquinaFactory` - 99 edges
9. `BodegaFactory` - 86 edges
10. `Sede` - 85 edges
...
### Community 0 - "cn"
Cohesion: 0.05
Nodes (83): AbstractUser, BodegaAdmin, BodegueroInline, CustomUserAdmin, LineaProduccionAdmin, TransformacionProductoAdmin, Carga ~1,000,000 registros para estresar el aplicativo TexCore.  Usa bulk_crea, Seed de SIMULACIÓN INTEGRAL de TexCore.  Puebla la base con un caso coherente (+75 more)
...
### Community 2 - "PagoCliente"
Cohesion: 0.03
Nodes (108): InventoryDashboardProps, StockItem, KardexViewProps, ManageAreas(), ManageAreasProps, AREA_1, SEDE_1, SEDE_2 (+100 more)

### Community 3 - "AdminSistemasDashboard.tsx"
Cohesion: 0.03
Nodes (35): get_user_role(), APIView, UserProfileView, CustomUserFactory, AreaViewSetTestCase, CustomUserViewSetTestCase, TestCase, Pruebas de gestion/views/core_views.py — SedeViewSet, AreaViewSet, CustomUserVi (+27 more)

### Community 4 - "AuditLog"
Cohesion: 0.07
Nodes (51): AuditLogger, AuditLogger: registra accesos de servicios internos. ISO 27001 A.12.4 — Registro, Registra en log estructurado (RFC 5424) cada acceso a la API interna     con ide, Emite log estructurado para trazabilidad ISO 27001 A.12.4.          Args:, IsInternalService, BasePermission, Permite acceso solo si el request fue autenticado como ServicePrincipal., AuditLoggerTestCase (+43 more)

### Community 5 - "factories.py"
Cohesion: 0.03
Nodes (96): Elimina toda la población de datos creada para pruebas (stress_test_data y seed_, Command, BaseCommand, Artefacto RUP: Módulo de Servicio Caso de Uso: CU-DescargaQuimicaAutomatica Pa, DetalleHistorialDespachoAdmin, DetalleHistorialDespachoInline, HistorialDespachoAdmin, MovimientoInventarioAdmin (+88 more)

### Community 6 - "AreaFactory"
Cohesion: 0.10
Nodes (76): Group, StockItem, AuditoriaDialogProps, EditarMovimientoDialogProps, EliminarMovimientoDialog(), EliminarMovimientoDialogProps, ItemIncompleto, ScannedItem (+68 more)

### Community 7 - "main.py"
Cohesion: 0.11
Nodes (28): COLORS, Props, ESTADO_BADGE, ManageLineasProps, ESTADO_BADGE, ManageMaquinasProps, AlertDialog(), AlertDialogAction() (+20 more)

### Community 8 - ".registrar_transformacion"
Cohesion: 0.06
Nodes (18): OrdenProduccionEstadoSerializer, OrdenProduccionFactory, DescargaQuimicosValidacionesTestCase, TestCase, Pruebas de validaciones de entrada de DescargaQuimicosService.descargar_para_op, TestCase, Tests de OrdenProduccion — máquina de estados. Aplica técnica ISTQB: State Tran, ISTQB STT — Transiciones legales en la máquina de estados. (+10 more)

### Community 9 - "TintoreroRBACTestCase"
Cohesion: 0.05
Nodes (24): ClienteFactory, SedeFactory, TestCase, Tests de Cliente — límite de crédito. Aplica técnicas ISTQB:   - Partición de, ISTQB EP — Clases de equivalencia para limite_credito:       Válida:   limite_c, BVA: valor mínimo exacto del límite., EP Clase Válida: cualquier valor positivo., BVA: valor alto permitido por el campo (max_digits=12, decimal_places=3). (+16 more)

### Community 10 - "UnifiedBusinessLogicTestCase"
Cohesion: 0.03
Nodes (42): Si la máquina del lote tiene producto_merma configurado y         peso_merma > 0, Revierte el stock de merma creado por este lote., LoteProduccionFactory, MaquinaConMermaFactory, MaquinaFactory, Meta, ParoMaquinaFactory, Factories para tests de TexCore — usa factory_boy.  Convención: - Cada factor (+34 more)

### Community 11 - "HasScope"
Cohesion: 0.04
Nodes (26): BodegaFactory, StockBodegaFactory, MovimientoMermaCreateTestCase, TestCase, Pruebas de control de Merma en MovimientoInventarioViewSet.create() — `tipo_movi, MovimientoDestroyTestCase, TestCase, MovimientoCreateTestCase (+18 more)

### Community 12 - "App.tsx"
Cohesion: 0.20
Nodes (6): PrecisionDecimalInventoryTestCase, TestCase, El almacenamiento de inventory debe conservar 3 decimales., BVA: 10.125 kg debe almacenarse exacto, no redondear a 10.13., BVA: cantidad y saldo_resultante con 3 decimales exactos., STT acumulación: 10 movimientos de 1.111 kg deben sumar 11.110 exacto.

### Community 13 - "EjecutivosDashboard.tsx"
Cohesion: 0.05
Nodes (72): AdminSedeDashboard(), InventoryDashboard(), ManageClientesProps, RegistrarEntradaView, ReportesView, SedeStatsDeps, TransferView, AlertaStock (+64 more)

### Community 14 - ".handle"
Cohesion: 0.03
Nodes (61): Proveedor, CostoHoraMaquina, CostoLoteProduccion, Tarifa vigente de un operario; vigente_hasta NULL = sin fecha de fin., Costo operativo por hora de maquina (amortizacion + energia + mantto)., TarifaOperario, ConsumoMateriaPrima, MateriaPrimaLote (+53 more)

### Community 15 - "ConsumoMezclaServiceConsumir"
Cohesion: 0.18
Nodes (6): Prueba que el despacho falle si no se proporcionan lotes., Suite única de pruebas para validar el funcionamiento del sistema TexCore., Verifica que solo vendedores/admins puedan cambiar el beneficio del cliente., Valida que el saldo_resultante se calcule correctamente., Versión Renombrada para asegurar detección.         Prueba que al registrar pro, UnifiedBusinessLogicTestCase

### Community 16 - "StockBodega"
Cohesion: 0.12
Nodes (21): BuildBalanceMasasPayloadTestCase, Función pura: reconcilia stock actual + movimientos del mes por producto.     C, _audit(), BalanceMasasPdfView, _build_balance_masas_payload(), _get_printing_url(), _now_iso(), _proxy_pdf() (+13 more)
...
Cohesion: 0.05
Nodes (42): AppContent(), AuditoriaDialog(), LOG_1, mockGet, AlertasStockView(), BodegueroDashboard(), EditarMovimientoDialog(), mockDelete (+34 more)

### Community 19 - "ServiceTokenRefreshViewTestCase"
Cohesion: 0.07
Nodes (17): Servicio de lectura que estructura la trazabilidad de una OP., Devuelve un dict con los pasos de transformación, mermas y el enlace a la, Encadena con la OP de la siguiente área vía TransferenciaInterarea.          Cor, TrazabilidadService, _crear_transf(), TestCase, Tests de TrazabilidadService — reconstrucción del flujo completo de producción., TrazabilidadServiceTest (+9 more)

### Community 20 - "DespachoReversionFKTestCase"
Cohesion: 0.03
Nodes (64): CostoLoteProduccionSerializer, RegistrarLoteProduccionSerializer, ConsumoMezclaService, SRP: gestiona el consumo de múltiples lotes de entrada para producir un lote de, EventoEtiquetaService, EventoEtiquetaService: orquesta la numeración y creación de eventos de etiqueta., MermaStockService, SRP: gestiona el stock de merma vendible por máquina.     ISO 27001 A.12.4: cada (+56 more)

### Community 21 - "BodegaFactory"
Cohesion: 0.03
Nodes (40): clear_cascade_justification(), Justificación para borrados en cascada (ej. DetalleFormula al borrar FormulaColo, set_cascade_justification(), Meta, ProductoSerializer, ProveedorSerializer, GroupSerializer, Meta (+32 more)

### Community 22 - "pdf_produccion_views.py"
Cohesion: 0.02
Nodes (146): react, AccordionContent(), AccordionItem(), AccordionTrigger(), Avatar(), AvatarFallback(), AvatarImage(), BreadcrumbEllipsis() (+138 more)

### Community 23 - "EjecutivosDashboard.test.tsx"
Cohesion: 0.08
Nodes (12): Command, BaseCommand, DetallePedidoViewSet, PedidoVentaViewSet, P0-005: pago + reconciliación en una sola transacción, con lock         pesimis, Artefacto RUP: Eliminación de Pago con Reversión         Caso de Uso: CU-Revers, Artefacto RUP: Acción de Reversión de Pago (alternativa a DELETE)         Caso, Nota de venta en PDF. Por defecto lista TODOS los detalles del pedido         ( (+4 more)

### Community 24 - "InventoryDashboard.reportes.test.tsx"
Cohesion: 0.10
Nodes (22): get_current_ip(), get_current_user(), AuditLog, _get_object_sede_id(), Validaciones de negocio que requieren contexto de auditoría.         Se llama a, Obtiene sede_id del objeto para filtrar logs de entidades eliminadas.     Prior, audit_user_delete(), audit_user_save() (+14 more)

### Community 25 - "ValidateResponse"
Cohesion: 0.05
Nodes (14): CustomUserSerializer, Ejecutivos tienen acceso a todo el dashboard de stock: asignar todas las bodegas, DetalleFormulaSerializer, ComponenteMezclaOPSerializer, DetallePedidoSerializer, ClienteSerializerValidateTieneBeneficioTestCase, ComponenteMezclaOPSerializerTestCase, CustomUserSerializerCreateUpdateTestCase (+6 more)

### Community 26 - "Auditoría y Mejoras del Rol Jefe de Área"
Cohesion: 0.06
Nodes (21): Environment, get_pdf_strategy(), generate_zpl_label(), get_zpl_strategy(), AuditRepository, BackgroundTasks, Genera texto ZPL a partir de un template Jinja2., ZplOutputStrategy (+13 more)
...
Cohesion: 0.06
Nodes (17): AreaFactory, ComponenteMezclaOPFactory, ConsumoLoteDetalleFactory, AreaProcessStepViewSetTestCase, ComponenteMezclaOPViewSetTestCase, ConsumoLoteDetalleViewSetTestCase, EtapaProduccionViewSetTestCase, OrdenProduccionCreateTestCase (+9 more)

### Community 29 - "MovimientoInventarioUpdateSerializer"
Cohesion: 0.03
Nodes (80): AdminSistemasDashboard(), ManageBodegas(), ManageClientes(), CLIENTE_1, CLIENTE_2, SelectCtx, toastErrorMock, ManageFormulas() (+72 more)
...
Cohesion: 0.07
Nodes (33): build_scan_record(), get_audit_repo(), AuditRepository de scanning_service: persiste eventos de auditoría en SQLite. DI, Factory function — construye ScanAuditLog desde los datos del dominio.     SRP:, Dependency provider para FastAPI Depends. DIP: el router no construye el repo., AuditRepository, BackgroundTasks, Router HTTP para validación de lotes. DIP: get_validation_service y get_audit_r (+25 more)

### Community 32 - "JefeAreaDashboard.tsx"
Cohesion: 0.05
...
### Community 33 - "ReportingViewsExtraTestCase"
Cohesion: 0.05
...
### Community 34 - "EtiquetaRequest"
Cohesion: 0.18
Nodes (6): PrintingServiceNotaVentaPdfTestCase, PrintingServiceUrlResolutionTestCase, PrintingServiceZplTestCase, TestCase, Pruebas de gestion/utils.py — PrintingService (proxy HTTP a printing_service)., Antes PRINTING_SERVICE_URL era una constante de módulo leída de     os.environ
...
Cohesion: 0.07
Nodes (51): Configuración centralizada del printing_service. Elimina dependencias de cwd (f, build_print_record(), get_audit_repo(), AuditRepository de printing_service: persiste eventos de auditoría en SQLite. DI, Factory function — construye PrintAuditLog. SRP: separa construcción de persiste, Dependency provider para FastAPI Depends. DIP: el router no construye el repo., generate_balance_masas_pdf(), generate_etiqueta_pdf() (+43 more)

### Community 37 - "Documentación: Descarga Automática de Químicos en Tintorería"
Cohesion: 0.04
Nodes (50): dependencies, axios, class-variance-authority, clsx, cmdk, date-fns, embla-carousel-react, @hookform/resolvers (+42 more)

### Community 38 - "Plan de Estabilización, Consolidadación y Despliegue a Producción — TexCore"
Cohesion: 0.12
Nodes (12): Identidad de un microservicio autenticado. Inmutable post-creación.      `sede, ServicePrincipal, HasScope, Permisos para la API interna. ISP: una clase por responsabilidad de permiso. C, Verifica que el ServicePrincipal tiene el scope requerido.     Uso: permission_, HasScopeTestCase, IsInternalServiceTestCase, TestCase (+4 more)

### Community 39 - "_crear_pedido"
Cohesion: 0.13
...
Cohesion: 0.03
Nodes (58): calcularSaldoAcumulado(), normalizeBodegaKey(), TransferFormData, validateTransfer(), KardexViewImpl(), ReportesViewImpl(), TransferViewImpl(), BODEGA (+50 more)
...
### Community 44 - "LoteProduccion"
Cohesion: 0.16
Nodes (8): Command, BaseCommand, Ingreso directo de stock con Kardex (COMPRA) para químicos/insumos., Crea el despacho (HistorialDespacho + detalles + MovimientoInventario VENTA)., El tintorero crea la FormulaColor (en_pruebas -> aprobada) que usará la OP., El bodeguero corrige un movimiento -> genera AuditoriaMovimiento., Ejecuta el motor MRP -> RequerimientoMaterial + OrdenCompraSugerida., Simula el usuario autenticado para que el AuditLog atribuya la acción.
...
### Community 53 - ".reconcile_client_orders"
Cohesion: 0.18
...
### Community 57 - "get_user_role"
Cohesion: 0.07
Nodes (26): AREA_1, AREA_3, BODEGA_1, BODEGA_3, CLIENTE_1, FORMULA_1, GROUP_1, LOTE_1 (+18 more)
...
Cohesion: 0.04
Nodes (29): ParoMaquina, Registro de downtime de máquina, con reason code = las Seis Grandes Pérdidas, make_group_permission(), Permisos de acceso para TexCore.  Usa make_group_permission() para eliminar la, Factory para crear clases de permiso basadas en grupos de Django.      Otorga, ConsumoLoteDetalleSerializer, LineaProduccionSerializer, MaquinaSerializer (+21 more)
...
Cohesion: 0.13
Nodes (14): EtiquetaRequest, DTO de entrada para generación de etiqueta ZPL., LabelService, Genera las imágenes de código de barras y QR de una etiqueta de lote., Code128 (misma simbología que ^BCN en ZPL) como PNG en base64.         `write_t, QR (misma corrección de errores 'M' que ^BQN,2,5 en ZPL) como PNG en base64., Construye el contexto para el template PDF, con las imágenes ya         generad, EP — campos requeridos vs. opcionales de EtiquetaRequest. (+6 more)

### Community 64 - "Datos para Dashboard de Reportes Gerenciales (Ejecutivo)"
Cohesion: 0.09
Nodes (10): ClienteListSerializer, ClienteSerializer, _fecha_pedido_to_iso_utc(), PedidoVentaResumenSerializer, PedidoVentaSerializer, Serializer ligero para listados masivos (Admin/Vendedor Dashboard), P1-002: anticipo disponible = saldo_calculado negativo invertido., Convierte fecha_pedido a ISO UTC con Z para que el frontend muestre la hora loca (+2 more)
...
Cohesion: 0.12
Nodes (16): 21 de Agosto de 2026, 24 de Agosto de 2026, 25 de Agosto de 2026, 27 de Agosto de 2026, Agosto 2026, Bugs reales de despacho encontrados probando el flujo end-to-end (con logs reales, no simulados), Cierre del plan de testabilidad y cobertura ≥90% (frontend), Despacho parcial robusto — estado real, no todo-o-nada (+8 more)
...
Cohesion: 0.15
Nodes (10): ProduccionKPIService, Calcula KPIs de producción filtrados opcionalmente por sede.      Uso:, ProduccionKPIServiceHistorialProductoTest, ProduccionKPIServicePorProductoTest, ProduccionKPIServiceTendenciaTest, ProduccionKPIServiceTest, SimpleTestCase, TestCase (+2 more)
...
Cohesion: 0.15
Nodes (9): _extract_message(), Manejo de excepciones centralizado para TexCore.  Formato de respuesta de erro, Extrae un mensaje legible de la estructura de datos del error DRF., Handler de excepciones unificado.     Reemplaza los 4 patrones distintos que ex, texcore_exception_handler(), ExtractMessageTestCase, TestCase, Pruebas de gestion/exceptions.py — texcore_exception_handler y _extract_message. (+1 more)

### Community 77 - "VendedorDashboard.cobranza.test.tsx"
Cohesion: 0.13
Nodes (13): ExcelFormatter, _fecha_a_texto(), _prepare_df(), DataFrame, Response, Formateador de salida Excel. Extrae la lógica de excel_generator.py y la encaps, Normaliza el DataFrame para escritura Excel: fechas como texto, numéricos redond, Convierte DataFrames a archivos .xlsx descargables con formato TexCore. (+5 more)

### Community 78 - "ServiceCredential"
Cohesion: 0.08
...
### Community 79 - "_audit"
Cohesion: 0.16
Nodes (8): Command, BaseCommand, MRPEngine, Devuelve los detalles de una fórmula usando caché en memoria., Ejecuta el cálculo completo del MRP de forma optimizada:         1. Limpia dato, run_mrp(), MRPTest, TestCase

### Community 80 - "JefeAreaDashboard.test.tsx"
Cohesion: 0.09
...
Cohesion: 0.15
Nodes (11): DespachoEstadoService, DespachoEstadoService — determina el estado real de un PedidoVenta según lo efe, Suma, por producto_id, el peso de DetalleHistorialDespacho asignados a, Suma el peso requerido por producto_id a partir de los detalles del pedido., Determina qué estado debería tener el pedido AHORA MISMO según lo         realm, ProcessDespachoAPIView, APIView, Valida si un código de lote (barras) existe y tiene stock disponible. (+3 more)

### Community 86 - "main.py"
Cohesion: 0.08
Nodes (12): Resuelve la sede a consultar respetando el aislamiento.         Retorna (sede_i, _get_required_env(), APIView, Obtiene una variable de entorno requerida. Falla si no existe (Fail-Fast)., Valida que el path del reporte sea seguro.     Previene Path Traversal y acceso, ReportingProxyView, _validate_report_path(), GetRequiredEnvTestCase (+4 more)
...
Cohesion: 0.03
Nodes (48): Bodega, DjangoModelFactory, DescargaQuimicosService, Decimal, Template Method: paso de reversión dentro de ajustar_descarga_op.         Se us, Template Method: secuencia fija revertir → descargar.         Se usa al modific, Servicio de descarga automática de químicos al crear/modificar/eliminar órdenes, Verifica si el stock ha caído por debajo del mínimo.         Emite warning en l (+40 more)

### Community 91 - "Grafo de Conocimiento y Navegación del Código (Graphify)"
Cohesion: 0.07
Nodes (21): BaseAuthentication, JWTServiceAuthentication, Request, JWTServiceAuthentication: valida JWT RS256 de microservicios. ISO 27001 A.9.4 —, Genera un JWT RS256 firmado para autenticación entre servicios.         ISO 270, DRF Authentication backend para JWT de servicio (RS256).     Retorna None si no, JWTServiceAuthenticationExtraTestCase, TestCase (+13 more)

### Community 92 - "Guía de Despliegue en Producción — TexCore"
Cohesion: 0.10
Nodes (11): TestCase, Suite de pruebas para verificar el Control de Acceso basado en Roles (RBAC)., Verifica que sin login no haya acceso a nada, Matrix test para GET /api/inventory/movimientos/         Denegados: solo operar, Matrix test para POST /api/inventory/movimientos/ (escritura)         Permitido, Matrix test para POST /api/inventory/transferencias/ (escritura de stock), Matrix test para GET /api/inventory/bodegas/{id}/kardex/ (lectura)         Dene, Matrix test para /api/inventory/historial-despachos/         Permitidos: admin_ (+3 more)
...
### Community 94 - "test_printing_endpoints.py"
Cohesion: 0.44
...
### Community 98 - "TransicionTresFasesTestCase"
Cohesion: 0.06
Nodes (28): AreaSerializer, DosificacionSerializer, Serializer de entrada para el endpoint de calculo de dosificacion., calcular_dosificacion_gr_l(), calcular_dosificacion_pct(), DosificacionCalculator, Decimal, Servicio de calculo de dosificacion para formulas de tintoreria.  Metodos de c (+20 more)

### Community 99 - "ReportAuditLog"
Cohesion: 0.19
...
Cohesion: 0.14
Nodes (12): _get_user_audit_data(), Extrae datos auditables del usuario (solo PKs para FKs)., _FakeField, _FakeMeta, _FakeModeloConCampoRoto, _FakeUsuarioConCampoRoto, GetModelAuditDataTestCase, GetUserAuditDataTestCase (+4 more)

### Community 101 - "Junio 2026"
Cohesion: 0.11
Nodes (10): TestCase, IDOR: un no-admin SIN sede que envía ?sede_id ajeno queda con el         paráme, Si falta bodega_id en un reporte restringido, debe dar 400, Sin autenticación no hay acceso, Un bodeguero DEBE poder acceder a reportes de su bodega asignada, Un bodeguero NO DEBE poder acceder a reportes de una bodega no asignada, Un administrador puede acceder a CUALQUIER bodega, El catálogo de productos no requiere bodega_id para el bodeguero (+2 more)

### Community 102 - "Análisis Comparativo: Monolito con Servicios Satélites vs Monolito Puro en TexCore"
Cohesion: 0.16
...
### Community 104 - "reporting_proxy.py"
Cohesion: 0.12
Nodes (17): Arquitectura de Capas, Diagrama de flujo de una solicitud PDF, DIP — Inyección de dependencias vía FastAPI `Depends`, Endpoints, Fragilidad operacional eliminada (config.py), `GET /health`, Integración con Django, Patrones de Diseño Aplicados y su Justificación (+9 more)
...
Cohesion: 0.15
Nodes (13): ProduccionProductoItem, date, Decimal, Endpoint dedicado para la tendencia de 30 días (usado independientemente)., Producción agregada por producto en el rango — drill-down ejecutivo (CU-EJ-08)., Serie diaria de kg producidos de UN producto — gráfica de drill-down (CU-EJ-09)., Genera la serie temporal completa (incluyendo días sin producción = 0), Agrupa los lotes del rango por producto de salida de su OP. (+5 more)

### Community 110 - "EditarMovimientoDialog.test.tsx"
Cohesion: 0.16
Nodes (8): async_export_report(), Tarea asíncrona para generar reportes pesados sin bloquear a Gunicorn.     Llam, Tarea asíncrona para el cálculo completo del MRP (Material Requirements Planning, run_mrp_calculation(), AsyncExportReportTestCase, TestCase, Pruebas de gestion/tasks.py — tareas asíncronas Celery.  async_export_report:, RunMrpCalculationTestCase

### Community 111 - "README.md"
Cohesion: 0.12
Nodes (5): CustomTokenObtainPairViewTestCase, CustomTokenRefreshViewTestCase, LogoutViewTestCase, TestCase, Pruebas de gestion/custom_jwt_views.py — login/refresh/logout basados en JWT po
...
### Community 116 - "LoteProduccionViewSetTestCase"
Cohesion: 0.13
Nodes (12): OutputFormatter, DataFrame, Protocol, Response, Protocol para formateadores de salida (Strategy Pattern). OCP: agregar formato, Convierte un DataFrame en una Response HTTP descargable., IReportRepository, DataFrame (+4 more)

### Community 117 - "Microservicios Independientes — Plan de Implementación"
Cohesion: 0.24
Nodes (12): Response, Servicio que ejecuta un SP, maneja el caso de DataFrame vacío,     y delega el, Ejecuta el SP y retorna la Response formateada.         Si el DataFrame está va, ReportService, _make_formatter(), _make_repo(), DataFrame, Tests unitarios del ReportService. Usa mocks de IReportRepository y OutputFormat (+4 more)

### Community 118 - "package.json"
Cohesion: 0.23
Nodes (7): MateriaPrimaRegistroTestCase, TestCase, EP inválida: costo < 0 → ValidationError., EP duplicado: mismo (proveedor, lote, fecha) viola unique_together., Registro de entradas de MP — EP + BVA., EP válida: crea MP, suma stock y registra movimiento COMPRA., BVA límite: cantidad == 0 → ValidationError, nada persiste.

### Community 119 - "TransformacionAPIViewTestCase"
Cohesion: 0.19
Nodes (8): Registra las credenciales de los microservicios en la BD (ServiceCredential). D, Command, BaseCommand, Management command: crea ServiceCredentials para los microservicios si no existe, Meta, ServiceCredential: identidad de un microservicio autorizado. ISO 27001 A.9.2 — G, Representa la identidad de un microservicio que consume la API interna., ServiceCredential
...
Cohesion: 0.21
Nodes (7): Genera hash seguro del secret usando el hasher de Django (PBKDF2/bcrypt)., TestCase, Tests TDD para ServiceCredential. EP + STT., STT: transición activo → inactivo revoca acceso., EP: creación normal de una credencial de servicio., TestServiceCredentialCreacion, TestServiceCredentialEstado
...
Cohesion: 0.14
Nodes (8): DjangoReportRepository, DataFrame, JWTTokenManager, Extrae 'sp_GetKardexBodega' de 'EXEC sp_GetKardexBodega @BodegaID=?, ..., Ejecuta un SP (mapeado a endpoint REST) y retorna el resultado como DataFrame., Adapter que implementa IReportRepository via Django Internal API., Tests para DjangoReportRepository. EP por endpoint representativo., TestDjangoReportRepository

### Community 127 - "pdf.py"
Cohesion: 0.17
...
Cohesion: 0.20
Nodes (9): browserslist, development, production, eslintConfig, extends, name, private, type (+1 more)
...
### Community 133 - "clear_cascade_justification"
Cohesion: 0.25
...
### Community 135 - ".calcular_oee_area"
Cohesion: 0.11
...
### Community 136 - "TestJWTTokenManager"
Cohesion: 0.15
...
Cohesion: 0.18
Nodes (6): Suite de pruebas para verificar el Control de Acceso basado en Roles (RBAC)., Matrix test para /api/inventory/historial-despachos/         Permitidos: admin_, Matrix test para /api/inventory/stock/         Permitidos: Casi todos excepto o, Matrix test para /api/inventory/process-despacho/ (Endpoint crítico de escritura, Verifica que sin login no haya acceso a nada, RBACMatrixTestCase

### Community 141 - "EliminarMovimientoDialog.test.tsx"
Cohesion: 0.07
Nodes (19): AuditMiddleware, _extract_client_ip(), get_cascade_justification(), _is_trusted_proxy(), Verifica si una IP pertenece a una red de proxy de confianza., Extrae la IP real del cliente de forma segura.     Solo confía en X-Forwarded-F, Middleware para capturar la IP y el usuario de la petición actual,     guardánd, AuditMiddlewareCallTestCase (+11 more)
...
Cohesion: 0.24
Nodes (11): Serializers para la API interna. ISP: un serializer por caso de uso., ServiceTokenRefreshRequestSerializer, ServiceTokenRequestSerializer, _generate_token_pair(), APIView, Endpoints de autenticación servicio-a-servicio. SRP: solo emite y renueva JWT de, POST /api/internal/v1/auth/refresh/ — renueva access token con refresh token., Genera par access+refresh JWT RS256. Lógica extraída para reutilización. (+3 more)
...
### Community 152 - "AreaNombreValidatorTestCase"
Cohesion: 0.14
...
Cohesion: 0.06
Nodes (43): BuscadorLotes(), ROLES_SUPERVISOR, mockGet, mockRole, SelectCtx, toastErrorMock, EmpaquetadoDashboard(), PackagingFormValues (+35 more)
...
Cohesion: 0.21
Nodes (5): OptionalPagination, PageNumberPagination, OptionalPaginationTestCase, TestCase, Pruebas de gestion/pagination.py — OptionalPagination.  Extiende PageNumberPag

### Community 159 - "2. Scripts SQL de Eliminación Directa"
Cohesion: 0.14
Nodes (3): FrontendLogViewTestCase, TestCase, Pruebas de las vistas de sistema — gestion/views/system_views.py.  FrontendLog

### Community 160 - "Guía de Comandos para Operaciones en Producción"
Cohesion: 0.27
Nodes (5): ProcessDespachoAPIViewTestCase, TestCase, Regresión directa del bug reportado: tras un primer despacho parcial,         u, Un despacho puede cubrir varios pedidos a la vez: cada lote escaneado         d, Regresión de bug real: revertir un despacho vía el endpoint HTTP         siempr
...
Cohesion: 0.09
Nodes (6): BuildReporteAvancePayloadTestCase, APITestCase, Función pura (sin ORM en su firma): mapea registros ya materializados     (dict, TestPdfProduccionViews, _build_reporte_avance_payload(), Traduce los registros del ORM al schema ReporteAvanceRequest del     printing_s

### Community 173 - "Análisis por Rol"
Cohesion: 0.28
...
Cohesion: 0.24
Nodes (7): _make_token(), _mock_httpx_async_client(), Tests para JWTTokenManager. EP + BVA., Genera un par RSA temporal para tests., Mock utilizable como reemplazo de httpx.AsyncClient — soporta     `async with ht, rsa_keys(), TestJWTTokenManager

### Community 176 - "get_current_user"
Cohesion: 0.17
...
### Community 180 - "FaseRecetaFactory"
Cohesion: 0.17
...
Cohesion: 0.21
Nodes (12): BODEGAS, completarFormulario(), destinoSection(), mockPost, origenSection(), PRODUCTOS, seleccionarDestino(), seleccionarOrigen() (+4 more)
...
### Community 187 - "BodegueroDashboard.test.tsx"
Cohesion: 0.20
...
### Community 191 - "test_database_engine.py"
Cohesion: 0.18
Nodes (10): devDependencies, vitest, name, private, scripts, build, dev, test (+2 more)

### Community 192 - "JWTServiceAuthenticationExtraTestCase"
Cohesion: 0.17
...
### Community 194 - "scanning_views.py"
Cohesion: 0.14
...
### Community 200 - "Documento de Contexto y Decisión Arquitectónica: TexCore — Sistema de gestión de órdenes de producción"
Cohesion: 0.18
...
Cohesion: 0.17
Nodes (9): ManageMaquinas(), MAQUINA_1, mockDelete, mockGet, mockPatch, mockPost, SelectCtx, toastErrorMock (+1 more)

### Community 205 - "manifest.json"
Cohesion: 0.17
Nodes (8): InventoryForm(), BODEGAS, mockGet, mockPost, PRODUCTOS, SelectCtx, toastErrorMock, toastSuccessMock

### Community 206 - "ProduccionKPIService"
Cohesion: 0.08
Nodes (48): ApprovalRequests(), SedesSidebarImpl(), getSedeStats(), KpiCard, KpiCardProps, COLORS, AlertasInventarioPanel, AlertasInventarioPanelImpl() (+40 more)
...
Cohesion: 0.25
Nodes (4): Replica las transiciones del OrdenProduccionSubprocesoViewSet., OP-SIM-001 finalizada: abastecimiento, descarga química, avance, transformación,, El empaquetado completa los subprocesos y empaca el lote (etiqueta lista)., OP-SIM-003 en proceso (avance en vivo) y OP-SIM-004 pendiente.
...
### Community 218 - "7. TESTING — ESTÁNDAR ISTQB"
Cohesion: 0.20
...
### Community 219 - "EjecutivosDashboard.reportes.test.tsx"
Cohesion: 0.20
...
Cohesion: 0.20
Nodes (7): KPI, LOTE_HOY, MAQUINA, mockGet, PRODUCTO_BAJO_STOCK, PROFILE, toastErrorMock
...
### Community 229 - "5. CALIDAD DE CÓDIGO — PRINCIPIOS SOLID Y DRY"
Cohesion: 0.38
...
Cohesion: 0.50
Nodes (4): crearSedeSinSeleccionarla(), mockEndpoints(), mockFullSedeData(), renderAt()

### Community 247 - "Autenticacion JWT RS256 entre Servicios"
Cohesion: 0.67
Nodes (3): Regresión de concurrencia del microservicio completo.  El Dockerfile levanta pri, _slow_render(), test_pdf_dado_peticiones_concurrentes_cuando_render_es_lento_entonces_no_se_serializan()

### Community 248 - "Scripts y Comandos de Gestion"
Cohesion: 0.29
Nodes (7): scripts, build, dev, preview, test, test:coverage, test:watch

### Community 249 - "10. RENDIMIENTO Y BASE DE DATOS"
Cohesion: 0.33
...
### Community 251 - "TransferenciasInterarea.test.tsx"
Cohesion: 0.22
...
### Community 254 - "_get_object_sede_id"
Cohesion: 0.25
...
### Community 261 - "20 de Julio de 2026"
Cohesion: 0.25
...
### Community 263 - "test_produccion_errores.py"
Cohesion: 0.25
...
### Community 265 - "Scripts de Utilidad"
Cohesion: 0.25
...
### Community 266 - "seed_production_masters.py"
Cohesion: 0.25
...
Cohesion: 0.20
Nodes (6): OpsEstado, ProduccionKPIs, RUP - Capa de Servicio: ProduccionKPIService ==================================, Punto de entrada único — Fachada sobre los métodos privados., Distribución de Órdenes de Producción por estado., Contrato de salida del servicio. Inmutable para garantizar consistencia     ent
...
### Community 279 - "RFC5424Formatter"
Cohesion: 0.29
...
### Community 283 - "TestPdfProduccionViews"
Cohesion: 0.29
...
Cohesion: 0.16
Nodes (8): LoteProduccionSerializer, LoteProduccionSerializerValidateDesviacionOrdenTestCase, LoteProduccionSerializerValidateEmpaquetadoTestCase, _payload(), TestCase, Pruebas de gestion/serializers/production_serializers.py — LoteProduccionSerial, CustomUserViewSet, Lista vendedores para filtros en dashboards (ejecutivo/admin).

### Community 291 - "TestValidateLoteView"
Cohesion: 0.33
...
### Community 294 - ".test_despacho_atomicidad"
Cohesion: 0.33
...
### Community 295 - "TestDocumentService_FormatearFecha"
Cohesion: 0.33
...
Cohesion: 0.29
Nodes (5): KardexView, mockUseKardex, MOV_ENTRADA, MOV_SALIDA, SelectCtx
...
### Community 316 - "13. MÉTRICAS DE DEUDA TÉCNICA"
Cohesion: 0.40
...
### Community 317 - ".test_empaquetado_flujo_avanzado"
Cohesion: 0.40
...
### Community 322 - "Nginx API Gateway"
Cohesion: 0.40
...
### Community 323 - "Scripts de Test Manual"
Cohesion: 0.40
...
### Community 324 - "verificar_auditoria.py"
Cohesion: 0.40
...
### Community 329 - "TestValidateLoteView"
Cohesion: 0.40
...
### Community 345 - "Protocol"
Cohesion: 0.67
Nodes (3): Regresión de concurrencia del microservicio completo.  El Dockerfile levanta rep, _slow_execute_sp(), test_export_dado_peticiones_concurrentes_cuando_backend_es_lento_entonces_no_se_serializan()

### Community 346 - "Response"
Cohesion: 0.50
...
### Community 352 - ".test_new_credit_terms_due_date_calculation"
Cohesion: 0.50
...
### Community 353 - ".test_salesman_filtering"
Cohesion: 0.50
...
### Community 354 - ".test_kpi_endpoint"
Cohesion: 0.50
...
### Community 355 - ".test_registrar_lote_empaquetado_completo"
Cohesion: 0.50
...
### Community 356 - ".test_salesman_order_filtering"
Cohesion: 0.50
...
- **Why does `CustomUserFactory` connect `AdminSistemasDashboard.tsx` to `cn`, `factories.py`, `.registrar_transformacion`, `TintoreroRBACTestCase`, `UnifiedBusinessLogicTestCase`, `HasScope`, `LineaProduccionViewSetTestCase`, `Hotspots Detallados`, `ServiceTokenRefreshViewTestCase`, `ValidateResponse`, `Documentación Técnica: Sistema de Reversión de Pagos del Cliente`, `NotaVentaRequest`, `Plan de Estabilización, Consolidadación y Despliegue a Producción — TexCore`, `_crear_pedido`, `Diseño: Microservicios Independientes — TexCore`, `get_session_factory`, `LoteValidationService`, `catalog_views.py`, `OperarioDashboard.test.tsx`, `ExecutiveKPIService`, `ReportAuditLog`, `README.md`, `._tendencia_diaria`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `SedeFactory` connect `TintoreroRBACTestCase` to `._tendencia_diaria`, `TransicionTresFasesTestCase`, `AdminSistemasDashboard.tsx`, `ReportAuditLog`, `factories.py`, `_crear_pedido`, `.registrar_transformacion`, `UnifiedBusinessLogicTestCase`, `HasScope`, `Diseño: Microservicios Independientes — TexCore`, `LineaProduccionViewSetTestCase`, `ValidateResponse`, `ExecutiveKPIService`, `NotaVentaRequest`, `LoteValidationService`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `StockBodega` connect `factories.py` to `cn`, `InventoryDashboard.tsx`, `AuditLog`, `UnifiedBusinessLogicTestCase`, `HasScope`, `App.tsx`, `.handle`, `StockBodega`, `DespachoReversionFKTestCase`, `BodegaFactory`, `Documentación Técnica: Sistema de Reversión de Pagos del Cliente`, `LoteProduccion`, `SedeFactory`, `catalog_views.py`, `ExecutiveKPIService`, `Grafo de Conocimiento y Navegación del Código (Graphify)`, `Análisis Comparativo: Monolito con Servicios Satélites vs Monolito Puro en TexCore`, `report_service.py`, `._tendencia_diaria`, `__init__.py`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Are the 104 inferred relationships involving `CustomUserFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`CustomUserFactory` has 104 INFERRED edges - model-reasoned connections that need verification._
- **Are the 95 inferred relationships involving `SedeFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`SedeFactory` has 95 INFERRED edges - model-reasoned connections that need verification._
- **Are the 78 inferred relationships involving `ProductoFactory` (e.g. with `AreaViewSetTestCase` and `ChemicalViewSetTestCase`) actually correct?**
  _`ProductoFactory` has 78 INFERRED edges - model-reasoned connections that need verification._
- **Are the 54 inferred relationships involving `OrdenProduccionFactory` (e.g. with `ConsumoMezclaServiceConsumir` and `ConsumoMezclaServiceRevertir`) actually correct?**
  _`OrdenProduccionFactory` has 54 INFERRED edges - model-reasoned connections that need verification._