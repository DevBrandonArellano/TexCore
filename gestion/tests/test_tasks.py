...
async_export_report: proxy asíncrono hacia reporting_excel (JWT RS256).
...
class AsyncExportReportTestCase(TestCase):
    ...
    def test_export_dado_respuesta_200_cuando_ejecuta_entonces_success(self, mock_token, mock_client_cls):
        ...
        resultado = task.run(report_path='kardex.xlsx', params={'a': 1}, user_id=7)

        self.assertEqual(resultado['status'], 'SUCCESS')
        self.assertEqual(resultado['report_path'], 'kardex.xlsx')
        self.assertEqual(resultado['user_id'], 7)

    @patch('gestion.tasks.httpx.Client')
    @patch('gestion.tasks.JWTServiceAuthentication.generate_token', return_value='fake-token')
    def test_export_dado_respuesta_error_http_cuando_ejecuta_entonces_reintenta(self, mock_token, mock_client_cls):
        ...
        with patch.object(task, 'retry', side_effect=Exception('retry-called')) as mock_retry:
            with self.assertRaises(Exception):
                task.run(report_path='kardex.xlsx', params={}, user_id=1)
    ...
    def test_export_dado_error_de_conexion_cuando_ejecuta_entonces_reintenta_con_excepcion(
        self, mock_token, mock_client_cls,
    ):
