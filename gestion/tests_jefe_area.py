...
class JefeAreaLogicTest(TestCase):
    def setUp(self):
        ...
        # Users
        self.jefe_tintoreria = CustomUser.objects.create_user(
            username='jefe_t', password='password', area=self.area_tintoreria, sede=self.sede)
        self.jefe_tintoreria.groups.add(self.group_jefe)

        self.operario1 = CustomUser.objects.create_user(
            username='op1', password='password', area=self.area_tintoreria, sede=self.sede)
    ...
    def test_jefe_area_queryset_filtering(self):
        ...
        # Jefe + Operario = 2
        users_list = response.data['results'] if isinstance(
