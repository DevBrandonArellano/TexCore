# Análisis Estratégico del Sector Textil

Este documento contiene análisis visuales y estratégicos sobre la competitividad y el posicionamiento de la industria textil ecuatoriana, contexto en el cual opera TexCore.

## 1. Análisis FODA (SWOT)

```mermaid
quadrantChart
    title Análisis Estratégico Sector Textil
    x-axis "Negativo" --> "Positivo"
    y-axis "Externo" --> "Interno"
    quadrant-1 "Fortalezas"
    quadrant-2 "Debilidades"
    quadrant-3 "Amenazas"
    quadrant-4 "Oportunidades"
    
    "Calidad de Confección": [0.8, 0.8]
    "Rapidez de Entrega (Regional)": [0.7, 0.7]
    "Dolarización (Estabilidad Compra)": [0.6, 0.9]
    "Conocimiento del Mercado Local": [0.8, 0.6]
    
    "Costos de Producción Altos": [0.2, 0.8]
    "Tecnología Rezagada": [0.3, 0.7]
    "Escasa Innovación (I+D)": [0.2, 0.6]
    "Dependencia de Importaciones": [0.3, 0.9]
    
    "Contrabando Masivo": [0.2, 0.2]
    "Competencia China (Precios)": [0.1, 0.3]
    "Inestabilidad Arancelaria": [0.3, 0.4]
    "Reformas Laborales Rígidas": [0.4, 0.3]
    
    "Nichos de Moda Sostenible": [0.7, 0.3]
    "E-commerce / Exportación Digital": [0.8, 0.2]
    "Acuerdos Comerciales (UE/USA)": [0.6, 0.4]
    "Demanda de Uniformes Técnicos": [0.7, 0.4]
```

### Detalle del Análisis

**Fortalezas (Interno + Positivo)**
*   **Calidad de Confección**: Reconocimiento de buena mano de obra.
*   **Rapidez de Respuesta**: Tiempos de entrega más cortos que Asia para el mercado local y andino.
*   **Estabilidad Monetaria**: La dolarización facilita la planificación de compra de maquinaria e insumos.

**Debilidades (Interno + Negativo)**
*   **Altos Costos**: Mano de obra y energía costosa comparada con competidores regionales.
*   **Tecnología**: Parque industrial con maquinaria antigua que reduce eficiencia.
*   **Insumos**: Falta de producción local de fibras sintéticas y químicos especializados.

**Oportunidades (Externo + Positivo)**
*   **Nuevos Mercados**: Tendencia hacia el "Fast Fashion" regional y nichos sostenibles.
*   **Canales Digitales**: Venta directa al consumidor final mediante plataformas web.
*   **Contratación Pública**: Proveer uniformes y dotación al estado con preferencia a producción nacional.

**Amenazas (Externo + Negativo)**
*   **Contrabando**: Ingreso ilegal de mercadería sin pagar aranceles.
*   **Competencia Global**: Productos asiáticos a precios inferiores al costo de materia prima local.
*   **Entorno Legal**: Cambios constantes en leyes tributarias y laborales.

---

## 2. Diagrama de Ishikawa (Causa-Raíz)

Este diagrama analiza las causas raíz de la **Baja Competitividad en el Sector Textil**.

```mermaid
mindmap
  root((Baja Competitividad
  Sector Textil))
    Mano de Obra
      Altos costos laborales
      Rigidez laboral
      Escasez de técnicos especializados
    Maquinaria y Tecnología
      Equipos obsoletos
      Baja automatización
      Alto costo de mantenimiento
      Falta de repuestos locales
    Materia Prima
      Dependencia de importaciones
      Fluctuación precios internacionales
      Aranceles a insumos
    Entorno y Mercado
      Contrabando textil
      Competencia desleal (Asia)
      Mercado interno pequeño
      Inestabilidad política/económica
    Procesos y Métodos
      Falta de estandarización
      Poco control de calidad
      Baja innovación en diseño
      Informalidad en la producción
    Infraestructura
      Altos costos de energía eléctrica
      Logística y transporte costoso
      Falta de plantas de tratamiento de agua
```
